...
class GPTService {
  constructor() {
    this.url = [
      "https://bartai.org/wp-admin/admin-ajax.php", 
      "https://chatgbt.one/wp-admin/admin-ajax.php", 
      "https://chatgptt.me/wp-admin/admin-ajax.php"
    ];
  }
  ...
  async gptAudio(audioUrl, type = 0) {
    ...
    const response = await fetch(this.url[type], { method: "POST", body: data });
...
export default async function handler(req, res) {
  if (req.method === 'GET') {
    const { action, prompt, url, type } = req.query;
    ...
    try {
      ...
      return res.status(400).json({ error: 'Invalid parameters or missing required fields' });
    } catch (error) {
