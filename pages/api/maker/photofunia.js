...
const proxyBase = proxyUrl[Math.floor(Math.random() * proxyUrl.length)];
...
export default async function handler(req, res) {
  ...
  if (method === "GET" || method === "POST") {
    ...
    const data = method === "GET" ? query : body; // For POST, use body, for GET use query

    const { action, url, images, texts } = data;
