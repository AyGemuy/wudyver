...
const getFonts = async () => {
  const res = await fetch("https://api.memegen.link/fonts", { headers: { accept: "application/json" } });
  const fonts = await res.json();
  return Object.values(fonts).map((v) => v.id);
};

const getTemplates = async () => {
  const res = await fetch("https://api.memegen.link/templates", { headers: { accept: "application/json" } });
...
const createImageFromTemplate = async (templateId, atas, bawah) => {
  const res = await fetch("https://api.memegen.link/images", {
    ...
    body: JSON.stringify({ template_id: templateId, text: [atas, bawah] }),
  });
...
const createImage = async (bg, atas, bawah, font) => {
  const res = await fetch("https://api.memegen.link/images/custom", {
    method: "POST",
    headers: { accept: "application/json", "Content-Type": "application/json" },
    body: JSON.stringify({ background: bg, text: [atas, bawah], font, extension: "png" }),
  });
  return await res.json();
};
  ...
  if (top && bottom) {
    try {
      ...
      if (template) {
        ...
        return res.status(400).json({ error: "Parameter 'link' atau 'template' diperlukan." });
      }
    ...
    return res.status(400).json({ error: "Parameters 'top' dan 'bottom' diperlukan." });
  }
}
