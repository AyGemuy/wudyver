...
const generateMeme = async (id, top, bottom) => {
  if (!memeList.includes(id)) {
    throw new Error('ID meme tidak valid.');
  }

  const url = `https://apimeme.com/meme?meme=${id}&top=${top}&bottom=${bottom}`;
  ...
  try {
    ...
    throw new Error('Gagal mengambil meme.');
  }
};

export default async function handler(req, res) {
const { method } = req;
  const { id, top, bottom } = req.method === 'GET' ? req.query : req.body;

  if (!id || !top || !bottom) {
    return res.status(400).json({ error: "Parameter 'id', 'top', dan 'bottom' wajib ada." });
  }

  const index = memeList.indexOf(id);
  if (index === -1) {
    return res.status(400).json({ error: 'ID meme tidak valid.' });
  }

  try {
    const memeBuffer = await generateMeme(id, top, bottom);
    Promise.resolve(memeBuffer)
      .then(() => {
