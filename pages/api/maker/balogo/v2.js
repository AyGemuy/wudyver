...
export default async function handler(req, res) {
  try {
    const { textL, textR, x, y } = req.query;

    if (!textL || !textR) {
      return res.status(400).json({ error: 'Parameter "textL" dan "textR" diperlukan' });
    }
    ...
    const url = `https://wudysoft-maker.hf.space/balogo/v2?${params.toString()}`;
