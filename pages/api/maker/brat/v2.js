...
class BratGenerator {
  ...
  async create(text = "Brat", fontSize = "100", blur = "5") {
    try {
      const data = new URLSearchParams({ action: 'generate_brat_text', text, fontSize, blurLevel: blur });
      const { data: base64 } = await axios.post(this.url, data.toString(), { headers: this.headers });
...
export default async function handler(req, res) {
  try {
    const { text = "Brat", fontSize = "100", blur = "5" } = req.method === "GET" ? req.query : req.body;
