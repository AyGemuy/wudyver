...
const priceMaker = async (title, text, usd, button) => {
...
<div class="container">
  <div class="box">
    ...
    <div class="options">
      <ul>
        <li> <i class="fa fas fa-check">${desc}</i></li>
      </ul>
    </div>
...
price('${title}', '${desc}', '${usd}', '${button}').then(a => console.log(a));`;
...
export default async function handler(req, res) {
  const { method } = req;
  const { title, text, usd, button } = req.method === 'GET' ? req.query : req.body;
  ...
  try {
    ...
    console.error(error);
    return res.status(500).json({ error: 'Failed to generate price image' });
  }
}
