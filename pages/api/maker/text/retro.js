...
export default async function handler(req, res) {
  const { method } = req;
  const { text1, text2 } = req.method === 'GET' ? req.query : req.body;

  if (!(text1 || text2)) {
    return res.status(400).json({ error: 'Text1 and text2 parameter is required' });
  }
  ...
  try {
    ...
    console.error(error);
    return res.status(500).json({ error: 'Failed to generate retro image' });
  }
}
