...
async function balogo(textL = "蔚蓝", textR = "档案", x = "-15", y = "0", tp = false) {
  ...
  await page.goto('https://lab.nulla.top/ba-logo'); // Make sure the local server is running
...
export default async function handler(req, res) {
  const { method } = req;
  const { textL = "蔚蓝", textR = "档案", x = "-15", y = "0", tp = false } = req.method === 'GET' ? req.query : req.body;

  if (!(textL || textR)) {
    return res.status(400).json({ error: 'textL, textR parameter is required' });
  }
  ...
  try {
    ...
    console.error(error);
    return res.status(500).json({ error: 'Failed to generate balogo image' });
  }
}
