...
export default async function handler(req, res) {
  const { action, query = '', url = '', text = '' } = req.query;
  ...
  if (action === 'search' && query) {
    try {
      ...
      return res.status(500).json({ result: 'Error during search' });
    }
  }
  ...
  if (action === 'preset' && url) {
    try {
      ...
      return res.status(200).json({ result: presetData });
    } catch (error) {
      return res.status(500).json({ result: 'Error fetching preset data' });
    }
  }
  ...
  if (action === 'create' && text && url) {
    try {
      ...
      if (generatedBuffer) {
        ...
        return res.status(500).json({ result: 'Failed to generate result' });
      }
    } catch (error) {
      return res.status(500).json({ result: 'Error generating result' });
    }
  }

  return res.status(400).json({ result: 'Invalid action or missing parameters' });
}
