...
const oreoMaker = async (text) => {
  ...
  const oreoContent = `<!DOCTYPE html>
  <html lang="en">
  <head>
    <meta charset="UTF-8">
  ...
  const oreoScript = `const { chromium } = require('playwright');

  async function generateOreoText(text) {
    const browser = await chromium.launch();
    const page = await browser.newPage();

    try {
      const content = \`${oreoContent}\`;
      await page.setContent(content);
      const screenshotBuffer = await page.screenshot({ type: 'png' });
      await browser.close();
      return screenshotBuffer.toString('base64');
    } catch (error) {
      console.error('Error generating Oreo text:', error);
    } finally {
      await browser.close();
    }
  }

  generateOreoText('${encodedText}').then(a => console.log(a));`;

  const { output } = await runPlaywrightCode(oreoScript.trim());
  return output;
};
...
export default async function handler(req, res) {
  ...
  try {
    ...
    console.error(error);
    return res.status(500).json({ error: 'Failed to generate Oreo text image' });
  }
}
