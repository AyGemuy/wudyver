...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (type === 'v1') {
      ...
      result = $("div.column.two-third.column_column table tbody tr")
        .map((index, element) => ({
      ...
      const html = await response.text();
      const $ = cheerio.load(html);
      result = $("div.column.one.column_column")
        .map((index, element) =>
          $(element)
            .parent()
            .find("a")
      ...
      const { url } = req.query;
      if (!url) {
        return res.status(400).json({ error: 'URL is required for listVoice' });
      }
      const response = await fetch(url);
      const html = await response.text();
      const $ = cheerio.load(html);
      result = $("div.column.one.column_column table tbody tr")
        .map((index, element) => ({
          link: $(element).find("td a").attr("href"),
