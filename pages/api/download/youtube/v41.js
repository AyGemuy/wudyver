...
class YTDownloader {
  ...
  async search(query) {
    try {
      const response = await axios.post(this.urlSearch, { q: query }, { headers: this.headers });
  ...
  async getInfo(u) {
    try {
      const response = await axios.post(this.urlInfo, { u: u }, { headers: this.headers });
      return response.data;
    } catch (error) {
  ...
  async downloadVideo(pickedVideoId, ext, fid) {
    try {
      const response = await axios.post(this.urlDownload, { pickedVideoId, ctx: 'video', ext, fid }, { headers: this.headers });
  ...
  generateFileUrl(fileData) {
    const { fileName, ext } = fileData;
    return `https://dl.yt-downloaderz.com/f/${fileName}.${ext}`;
  }

  async downloadFile(fileUrl) {
    try {
      const response = await axios.get(fileUrl, { headers: this.headers, responseType: 'arraybuffer' });
      return response.data;
    } catch (error) {
...
export default async function handler(req, res) {
  const ytDownloader = new YTDownloader();
    ...
    const { action, query, url, fileName, ext, fid } = req.method === "GET" ? req.query : req.body;
    ...
    try {
      switch (action) {
        ...
        case 'get':
          ...
          const fileUrl = ytDownloader.generateFileUrl({ fileName, ext: ext || 'mp4' });
