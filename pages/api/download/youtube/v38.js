...
class VideoDownloader {
  ...
  async checkStatus(id) {
    try {
      ...
      const { data } = await axios.post(url, { data: id });
      return data;
    } catch (error) {
  ...
  async downloadVideo({ url, format = "mp4", mp3Quality = "128kbps", mp4Quality = "360p", host = "1" }) {
...
export default async function handler(req, res) {
  try {
    ...
    if (!params.url) return res.status(400).json({ error: "No URL" });
