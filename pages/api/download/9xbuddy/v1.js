...
class XBuddy {
  ...
  _b64Dec(e) {
    ...
    while (n < e.length) {
      ...
      switch (n % 4) {
        case 1: s.push(String.fromCharCode(r << 2 | t >> 4)); break;
        case 2: s.push(String.fromCharCode((15 & r) << 4 | t >> 2)); break;
        case 3: s.push(String.fromCharCode((3 & r) << 6 | t));
      }
      r = t; n++;
    }
    return s.join("");
  ...
  _b64Enc(e) {
    let t, r, n, s = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/", a = 0, o = [];
    while (a < e.length) {
      t = e.charCodeAt(a); n = a % 3;
      switch (n) {
        case 0: o.push(s.charAt(t >> 2)); break;
        case 1: o.push(s.charAt((3 & r) << 4 | t >> 4)); break;
        case 2: o.push(s.charAt((15 & r) << 2 | t >> 6)); o.push(s.charAt(63 & t));
      }
      r = t; a++;
    }
    if (n === 0) { o.push(s.charAt((3 & r) << 4)); o.push("=="); }
    else if (n === 1) { o.push(s.charAt((15 & r) << 2)); o.push("="); }
    return o.join("");
  }
  ...
  async init() {
    ...
    try {
      ...
      const { data: html } = await axios.get(`https://${this.cfg.home}/`, { headers: { "User-Agent": this.cfg.ua } });
  ...
  async _req(method, ep, data = {}, auth = true) {
    try {
      const h = { ...this.cfg.headers, "x-auth-token": this._genToken() };
      if (auth) {
        if (!this.state.token) {
      ...
      const res = await axios({ method, url: `${this.cfg.base}${ep}`, headers: h, data: Object.keys(data).length ? data : undefined });
      return res.data;
    } catch (e) {
      ...
      if (json.response.formats) {
        json.response.formats = json.response.formats.map(v => {
          if (!/^http|\/\/|\/convert/.test(v.url)) {
            try { v.url = this._crypt(this._hex2bin(v.url).split("").reverse().join(""), k, 'dec'); } catch (e) {}
          }
          return v;
        });
      }
  ...
  async info(url) {
    ...
    const res = await this._req("POST", "/extract", { url: encUrl, _sig: sig, searchEngine: "yt" });
  ...
  async _pick(formats, qual, convert) {
    ...
    // --- FIX: Filter HTTPS Only jika convert false ---
    if (!convert) {
  ...
  async _resolve(rawUrl) {
    ...
    if (u.includes("/convert/")) {
      ...
      const [, , uid, param] = u.split("/"); // simple split fix logic depend on structure
      // URL structure: /convert/<uid>/<base64url>
      const parts = u.split("/convert/")[1].split("/");
      
      await this._req("POST", "/convert", { uid: parts[0], url: parts[1] });
      for (let i = 0; i < 60; i++) {
        const p = await this._req("POST", "/progress", { uid: parts[0] });
  ...
  // === MAIN EXECUTOR ===
  async download({ url, quality, convert = false, output = 'url' }) {
    try {
      ...
      // 3. Logic Convert: FALSE
      if (!convert) {
        console.log("[EXEC] Returning RAW result (Convert False)");
        // Bersihkan hasil agar lebih rapih (buang buffer/internal)
        return {
          success: true,
          selected: sel,
          ...raw.response
      ...
      // 4. Logic Convert: TRUE (Resolve & Download)
      const finalUrl = await this._resolve(sel.url);
      
      if (output === 'url') {
        return { success: true, selected_format: sel.quality, url: finalUrl };
      }

      console.log("[EXEC] Downloading Content...");
      const { data: buf } = await axios.get(finalUrl, { 
        responseType: 'arraybuffer', 
        headers: { 'User-Agent': this.cfg.ua } 
      });
