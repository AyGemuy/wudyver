...
class Debrid {
  ...
  async decryptLang() {
    ...
    try {
      ...
      // Decrypt process (following the web code exactly)
  ...
  async download({ url, provider = 1, ...rest }) {
    ...
    try {
      ...
      // Step 2: First API request - following web code logic
      ...
      // Add additional params based on web code
      if (data.mode === 'plg') {
      ...
      // Handle error codes from web code
      if (response.error_code) {
        const errorMsg = data[`code_${response.error_code}`] || `Error code: ${response.error_code}`;
        console.error('API Error:', errorMsg);
        
        // For error_code 3, it might need special handling
        if (response.error_code === 3) {
          // Try alternative approach from web code
      ...
      // Step 4: Get final download link
      // Following web code: openLink function
      ...
      const dlRes = await this.client.post(
        `${this.provider}/dl?id=${dlId}`,
        dlForm.toString(),
        {
          maxRedirects: 0,
      ...
      if (!finalUrl) {
        // Try alternative: the link might be in the response body
  ...
  async alternativeDownload(url, data) {
    ...
    try {
      // Try the generateLink approach from web code
