    ...
    async downloadTrack(url) {
        ...
        try {
            const { data } = await this.client.post(
                'https://api.menarailpost.com/soundcloud/download',
            ...
            $('ul.my-4 li').each((_, element) => {
                ...
                if (link && link.startsWith('https')) {
                    downloads.push({ format, link });
                }
            });

            const downloadLinks = await Promise.allSettled(
                downloads.map(async ({ format, link }) => {
                    try {
            ...
            return {
                ...
                downloadLinks: downloadLinks
                    .filter(result => result.status === 'fulfilled' && result.value) // Hanya mengambil yang berhasil
                    .map(result => result.value) // Mengambil hasil yang valid
            };
        } catch (error) {
