...
class SoundCloudDownloader {
    ...
    async fetchFormToken() {
        try {
            const response = await this.client.get('https://soundcloudtomp3.biz/', { headers: this.headers });
    ...
    async downloadTrack(url, quality = '128') {
        try {
            ...
            const response = await this.client.post('https://soundcloudtomp3.biz/index.php', postData, { headers: this.headers });
            const $ = cheerio.load(response.data);
            ...
            // Extract track name, preview image, and mp3 file
            $('#preview').each((i, el) => {
                trackName = $(el).find('p').eq(2).text().trim() || $(el).find('p').eq(0).text().trim(); // Track name in the second <p> tag
                previewImage = $(el).find('img').eq(0).attr('src'); // First <img> tag
            });
