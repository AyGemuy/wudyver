...
class SoundCloudDownloader {
    ...
    async downloadTrack(songUrl) {
        try {
            ...
            const response = await this.client.post('https://www.forhub.io/download.php', postData.toString(), { headers });
            ...
            const tableData = $('table tbody tr').map((idx, el) => {
                ...
                return { trackImage, trackTitle, bitrate, downloadLink };
            }).get();
            
            const trackData = tableData;
const result = { ...trackData[0], downloadLink: trackData[1].downloadLink };
