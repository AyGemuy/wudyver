...
class SoundCloudDownloader {
    ...
    generateUserId() {
        return uuidv4(); // Generates a unique user ID using UUID v4
    }

    async downloadTrack(link) {
        const userId = this.generateUserId(); // Generate a new userId for each request

        try {
            const { data } = await this.client.get(this.apiUrl, {
                headers: {
                    ...this.headers,
                    'userid': userId // Use generated userId in request headers
                },
                params: {
