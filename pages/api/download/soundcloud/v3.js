...
class SoundCloudDownloader {
    ...
    async fetchCSRFToken() {
        try {
            const { data } = await this.client.get(this.apiUrl, { headers: this.headers });
    ...
    async downloadTrack(link) {
        ...
        if (!csrfToken) return { status: false, message: 'CSRF token missing' };

        try {
            const { data } = await this.client.post(this.apiUrl, `csrfmiddlewaretoken=${csrfToken}&soundcloud=${encodeURIComponent(link)}`, { headers: this.headers });
            const results = this.extractDownload(data);
            return results.length ? { status: true, results } : { status: false, message: 'No tracks found' };
        } catch {
