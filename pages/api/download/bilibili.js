...
class Bilibili {
  ...
  async getInfo(link) {
    try {
      ...
      const {
        data
      } = await axios.get(`https://api.bilibili.tv/intl/gateway/web/playurl?s_locale=id_ID&platform=html5_a&aid=${videoId}&qn=64&type=0&device=wap&tf=0&spm_id=bstar-web.ugc-video-detail.0.0&from_spm_id=`, {
        headers: headers
      });
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (!link) return res.status(400).json({
      error: "Link diperlukan"
    });
    const playUrl = await bilibili.getInfo(link);
    if (playUrl) return res.status(200).json({
      playUrl: playUrl
    });
