...
async function downloadDrakor(url) {
  try {
    const response = await fetch(url),
      html = await response.text(),
      $ = cheerio.load(html),
      genres = $('.genrenya span[rel="tag"]')
        .map(function (_, el) {
          return $(el).text().trim();
        })
        .get(),
      resolutions = $("thead th")
      ...
      downloadInfo: $("#content-post table.mdl-data-table tbody tr")
        .map(function (_, el) {
          return {
            ...
            episodeInfo: Object.fromEntries(
...
export default async function handler(req, res) {
  ...
  if (method === "GET") {
    ...
    if (search) {
      const searchResults = await searchDrakor(search);
      return res.status(200).json(searchResults);
    }

    if (url) {
      const details = await downloadDrakor(url);
