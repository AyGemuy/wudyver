...
const spowload = {
    create: async (link) => {
        ...
        try {
            ...
            const { data: html } = await client.get(meta);
            ...
            let redirect;
            try {
                const analyze = await client.post('https://spowload.com/analyze',
                    new URLSearchParams({ _token: csrfToken, trackUrl: link }),
                    { 
                        headers: {
                            ...headers,
            ...
            const { data: convert } = await client.post('https://spowload.com/convert',
                {
                    urls: link,
