...
class SpotifyDownloader {
    ...
    async download(url) {
        if (!url.includes('spotify.com')) {
            return { error: 'URL harus dari Spotify' };
        }
        ...
        try {
            ...
            if (!response.data.result) {
                return { error: 'Gagal mendapatkan informasi lagu' };
            }
            ...
            if (!track || !track.id || !result.gid) {
                return { error: 'ID lagu tidak ditemukan' };
            }
            ...
            if (!convertResponse.data.result) {
                return { error: 'Gagal memulai konversi' };
            }
            ...
            while (true) {
                ...
                if (!progressResponse.data.result) {
                    return { error: 'Gagal mengambil progres konversi' };
                }
                ...
                if (status === 3) {
                    ...
                    return { error: 'Konversi gagal' };
                }
            }
        } catch (error) {
            console.error(error);
            return { error: 'Terjadi kesalahan pada request' };
        }
    }
}
