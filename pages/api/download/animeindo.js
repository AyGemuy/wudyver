  ...
  async download(url) {
    try {
      ...
      const buttons = $('.servers a').map((_, el) => {
        ...
        return link && name ? { title: `Download: ${name}`, url: link } : null;
      }).get();
...
export default async function handler(req, res) {
  ...
  if (method === 'GET') {
    try {
      const { action, url, q } = query;
      
      switch (action) {
        case 'latest':
          const latestAnimes = await animeindo.latest();
          return res.status(200).json({ data: latestAnimes });
        ...
        case 'detail':
          if (!url) return res.status(400).json({ error: 'URL parameter "url" is required' });
          const animeDetail = await animeindo.detail(url);
          return res.status(200).json({ data: animeDetail });
        
        case 'download':
          if (!url) return res.status(400).json({ error: 'URL parameter "url" is required' });
          const downloadData = await animeindo.download(url);
          return res.status(200).json({ data: downloadData });
