...
class MediafireDownloader {
  ...
  async getFolderContents($) {
    ...
    $("li.row_container").each((_, el) => {
      const elmt = $(el);
      const name = elmt.find(".item-name").text().trim();
      const link = elmt.find(".foldername, .filetype_column").attr("href");
      const size = elmt.find(".file_maindetails .size").text().trim();
      const date = elmt.find(".file_maindetails .created").text().trim();
      const key = elmt.attr("data-key");
  ...
  async getFileDownloadLink($) {
    ...
    const keyMatch = link ? link.match(/\/([^/]+)\/[^/]+$/) : null;
    const key = keyMatch ? keyMatch[1] : "Unknown Key";
    if (!link) return {
