...
export default async function handler(req, res) {
  ...
  const { link, download } = query;

  if (!link) {
    return res
      .status(400)
      .json({ error: 'Parameter `link` wajib disertakan' });
  }

  const id = extractId(link);

  if (!id) {
    return res.status(400).json({ error: 'Masukin link terabox yang valid' });
  }

  try {
    const url = `https://api.sylica.eu.org/terabox/?id=${id}${
      download ? '&download=1' : ''
    }`;
    const { data } = await axios.get(url, { headers });

    return res.status(200).json({ result: response(data.data, Boolean(download)) });
  } catch (error) {
    console.error(error);
