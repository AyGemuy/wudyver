...
export default async function handler(req, res) {
  if (req.method !== "GET") return res.json({ error: "Only GET" });
  try {
    const { url } = req.body;
...
async function saveinsta(url) {
  try {
    ...
    const { data } = await axios.post("https://saveinsta.io/core/ajax.php", payload, {
      headers: {
    ...
    const media = $("div.row > div.col-md-12 > div.row.story-container.mt-4.pb-4.border-bottom")
      .map((_, el) => {
