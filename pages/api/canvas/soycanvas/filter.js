...
export default async function handler(req, res) {
  if (req.method === 'GET') {
    const { type = 'affect', image = 'https://png.pngtree.com/thumb_back/fw800/background/20230117/pngtree-girl-with-red-eyes-in-anime-style-backdrop-poster-head-photo-image_49274352.jpg', image2 = 'https://png.pngtree.com/thumb_back/fw800/background/20230117/pngtree-girl-with-red-eyes-in-anime-style-backdrop-poster-head-photo-image_49274352.jpg' } = req.query;

    if (!type || !image) {
      return res.status(400).json({ error: 'Missing required parameters: type or image' });
    }
    ...
    try {
      ...
      switch (type) {
        case 'affect':
          ...
          break;
        case 'batslap':
          if (!image2) return res.status(400).json({ error: 'Missing image2 for batslap' });
        ...
        case 'invert':
          ...
          break;
        case 'kiss':
          if (!image2) return res.status(400).json({ error: 'Missing image2 for kiss' });
