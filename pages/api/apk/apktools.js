...
class ApkTools {
    constructor() {
        ...
        this.baseHtmlV = `https://${process.env.DOMAIN_URL}/api/tools/web/proxy/v3?method=GET&url=`;
    ...
    async detail(url) {
        try {
            ...
            return { error: 'Gagal mengambil detail aplikasi' };
        }
    }
    ...
    async getVariant(link) {
        try {
            ...
            return await Promise.all(variants.map(async v => ({ ...v, ...(await this.getVariantDetail(v.link)) })));
        } catch (e) {
            return { error: 'Gagal mengambil varian aplikasi' };
        }
    }
    ...
    async getVariantDetail(link) {
        try {
            ...
            return { error: 'Gagal mengambil detail varian' };
        }
    }
}
