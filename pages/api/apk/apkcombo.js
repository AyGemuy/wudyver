...
class ApkCombo {
    ...
    async search(query) {
        try {
            ...
            const { data } = await axios.get(`${this.baseHtml}${url}`);
    ...
    async detail(url) {
        try {
            ...
            const oldLink = oldVersionsSection.find('.more a').attr('href') 
                ? `${this.baseUrl}${oldVersionsSection.find('.more a').attr('href')}` 
                : null;
    ...
    async getData(link) {
        try {
            ...
            const $ = cheerio.load(response.data);
            const infoMeta = this.extractMeta($);
            return { meta: infoMeta, version: $('.content-tab-latest-version .file-list li').map((_, el) => ({
