...
const proxyUrl = `https://${process.env.DOMAIN_URL}/api/tools/web/html/v1?url=`;
...
export default async function handler(req, res) {
  ...
  try {
    const result = action === "search" ? await searchHippo(app_name) : action === "get" ? await getHippo(app_name) : null;
