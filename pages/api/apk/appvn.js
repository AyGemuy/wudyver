...
async function searchAppVn(query) {
  try {
    ...
    const response = await fetch(randomProxyUrl + link + "/android/search?keyword=" + encodeURIComponent(query));
...
async function getAppVn(url) {
  try {
    ...
    const response = await fetch(randomProxyUrl + data.downloadLink);
    const html = await response.text();
    const $ = cheerio.load(html);
  ...
  if (method === 'GET') {
    ...
    try {
      if (action === 'search') {
        ...
        const { latestVersion, sopcastId, package_name, version_code } = req.body;
