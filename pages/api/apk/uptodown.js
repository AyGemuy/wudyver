...
export default async function handler(req, res) {
  const { query, action, slug } = req.method === "GET" ? req.query : req.body;
  const uptodown = new Uptodown();

  if (!action) {
    return res.status(400).json({ error: 'Parameter "action" wajib diisi (search atau download).' });
  }
  ...
  try {
    ...
    switch (action) {
      case "search":
        if (!query) return res.status(400).json({ error: 'Parameter "query" wajib diisi untuk pencarian.' });
        result = await uptodown.search(query);
        break;
      case "download":
        if (!slug) return res.status(400).json({ error: 'Parameter "slug" wajib diisi untuk unduhan.' });
        result = await uptodown.download(slug);
        break;
      default:
        return res.status(400).json({ error: 'Action tidak valid. Gunakan "search" atau "download".' });
    }
