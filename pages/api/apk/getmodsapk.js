...
export default async function handler(req, res) {
  ...
  if (method === 'GET') {
    ...
    try {
      if (action === 'searchGetmodsapk') {
        const results = await searchGetmodsapk(query);
        return res.status(200).json(results);
      } else if (action === 'getLinks') {
        const result = await getLinks(query); // Assuming query is the URL
        return res.status(200).json(result);
      } else if (action === 'getLinkList') {
        const result = await getLinkList(query); // Assuming query is the URL
        return res.status(200).json(result);
      } else if (action === 'getDown') {
        const result = await getDown(query); // Assuming query is the URL
        return res.status(200).json(result);
      } else {
