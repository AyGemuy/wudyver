...
class MailDaxApi {
  ...
  async create() {
    try {
      const response = await axios.post(this.baseUrl, null, { headers: this.headers });
...
export default async function handler(req, res) {
  const { action, email, secret } = req.method === "GET" ? req.query : req.body;
  ...
  switch (action) {
    ...
    case 'message':
      if (!email || !secret) {
        return res.status(400).json({ success: false, error: 'Missing email or secret' });
      }
      try {
