...
class EmailService {
  ...
  async checkMessages({
    ...
    try {
      ...
      const msgCount = $("#mess_number").text().trim();
      if (!msgCount || msgCount === "0") return {
        success: true,
        message: "Tidak ada pesan",
        emails: []
      };
      const messages = $("#email-table .list-group-item-info").map((_, el) => ({
        from: $(el).find(".from_div_45g45gg").text().trim(),
        subject: $(el).find(".subject_div_45g45gg").text().trim(),
        time: $(el).find(".time_div_45g45gg").text().trim(),
        message: $(el).next().find(".mess_bodiyy").text().trim()
      })).get();
      return {
        success: true,
        message: `Ditemukan ${msgCount} pesan`,
        emails: messages
      };
    } catch (error) {
      return {
