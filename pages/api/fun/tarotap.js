...
class Tarotap {
  async daily(
    ...
    const data = { name, locale, email, date };
    try {
      ...
      console.error(error);
      return { error: 'Failed to fetch daily fortune' };
    }
  }
  ...
  async yesno(locale = 'en', selectedValue = 'one-card', question = '') {
    ...
    const data = { locale, selectedValue, question };
    try {
      const response = await fetch(url, {
        method: 'POST',
        headers: headers,
      ...
      console.error(error);
      return { error: 'Failed to fetch yes/no response' };
    }
  }
}
...
export default async function handler(req, res) {
  ...
  if (action === 'daily') {
    const { name, locale, email, date } = req.query;
    const result = await tarotap.daily(name, locale, email, date);
    return res.status(200).json({ result });
  }

  if (action === 'yesno') {
    const { locale, selectedValue, question } = req.query;
