  ...
  async detail(url) {
    try {
      const html = await axios.get(url, this.header),
        $ = cheerio.load(html.data),
        genre = $($("div.spe").find("span")[9])
          .find("a")
          .map((i, e) => $(e).text().trim())
          .get(),
        h1 = $("div.download-content")
          .find("h1")
          .map((i, e) => $(e).text())
          .get(),
        h4 = $("div.download-content")
          .find("h4")
          .map((i, e) => $(e).text())
          .get(),
        link = $("div.download-content ul")
          .map((i, e) => {
            const url = $(e)
              .find("li a")
              .map((b, c) => ({
      ...
      if (episode.length === 0) {
        ...
        episode.push({
          episode: epsTitle[0], // Use the first epsTitle as the episode name
          link: link
            .filter((v) => v.index === 0) // Filter by index for the first episode
            .filter((v) => v.url.length !== 0),
        });
      }
      ...
      return {
        ...
        data: {
          ...
          status: $($("div.spe").find("span")[3])
            .text()
            .replace("Status Anime", "")
            .trim(),
          type: $($("div.spe").find("span")[2])
            .text()
            .replace("Tipe Anime", "")
            .trim(),
          release: $($("div.spe").find("span")[6])
            .text()
            .replace("Tanggal Rilis", "")
            .trim(),
          studio: $($("div.spe").find("span")[7])
            .text()
            .replace("Studio", "")
            .trim(),
          duration: $($("div.spe").find("span")[8])
            .text()
            .replace("Durasi per Episode", "")
            .trim(),
          genre: genre.join(", ").trim(),
          score: $($("div.spe").find("span")[10])
            .text()
            .replace("Skor", "")
            .trim(),
          views: $($("div.spe").find("span")[11])
            .text()
            .replace(new RegExp("Dilihat", "g"), "")
            .trim(),
