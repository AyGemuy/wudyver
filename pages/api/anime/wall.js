...
class Search {
  ...
  async search(query = 'Random', customLimit) {
    try {
      const { url, params } = this.searchTypes[this.site];
      const queryParams = {
        ...params,
      ...
      return /xml/.test(contentType) || /<\?xml.*?>/.test(response.data)
        ? await this.xmlToJson(response.data)
        : response.data;
    } catch (error) {
  ...
  async xmlToJson(xml) {
    try {
      const parseXml = util.promisify(
        new Parser({ explicitArray: false, mergeAttrs: true }).parseString
      );
...
export default async function handler(req, res) {
  ...
  if (method === 'GET') {
    const { action, site, query: searchQuery, limit } = query;
