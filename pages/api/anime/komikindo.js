...
class Komikindo {
  ...
  async search(query) {
    try {
      ...
      const results = $('.postbody .film-list .animepost')
        .get()
        .map((el) => {
          const $el = $(el);
          ...
          const ratingStyle = $el
            .find('.adds .rating .archiveanime-rating-bar span')
            .attr('style');
  ...
  async detail(url) {
    try {
      ...
      return { mangaData, spoilerImages, relatedPosts, chapters };
    } catch (err) {
...
export default async function handler(req, res) {
  ...
  if (method === 'GET') {
    try {
      const { action, searchQuery, url } = query;
      ...
      return res.status(400).json({ message: 'Invalid action or parameters' });
    } catch (error) {
