...
class AnilistClient {
  ...
  async populer() {
    try {
      ...
      const extractMediaCards = (selector) =>
        $(selector)
          .map((_, element) => {
      ...
      return {
        ...
        top: $(('.landing-section.top .results .media-card'))
          .map((_, element) => {
    ...
    try {
      ...
      return $('.media-card')
        .map((_, element) => {
          ...
          return title && imageUrl && link ? { title, imageUrl, link: `${this.baseURL}${link}` } : null;
        })
        .get();
    } catch (error) {
  ...
  async detail(url) {
    try {
      ...
      return {
        ...
        genres: $('div.data-set:contains("Genres") .value a')
