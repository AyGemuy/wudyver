...
class Gogoanime {
  ...
  async search(name, { page = '1' } = {}) {
    ...
    const htmlContent = await axios({
      url: `${BaseURL}/search.html?keyword=${name}&page=${page}`,
    });
    let $ = cheerio.load(htmlContent.data);
    const searchResults = [];
  ...
  async fetchAnime(link) {
    ...
    $('div.anime_info_body_bg p.type').each(function(i, elem) {
      ...
      if (/plot_summary|released|other_name/g.test(keyName)) animeData[keyName] = $(elem).html().replace(
        `<span>${$x('span').text()}</span>`,
        ''
      );
...
export default async function handler(req, res) {
  ...
  try {
    if (req.query.action === 'search') {
      const result = await gogoanime.search(req.query.name, { page: req.query.page });
