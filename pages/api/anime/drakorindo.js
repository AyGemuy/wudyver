...
class DrakorIndo {
  async search(query) {
    let { data } = await axios.get("https://drakorindo.im/?s=" + query);
  ...
  async ongoing(page = "1") {
    let { data } = await axios.get("https://drakorindo.im/genre/ongoing/page/" + page + "/");
    let $ = cheerio.load(data);
  ...
  async detail(url) {
    try {
      ...
      $('p:contains("Episode")').each((i, elem) => {
        ...
        downloadLinks.push({ episode, links });
      });
...
export default async function handler(req, res) {
  ...
  try {
    ...
    // Handle 'ongoing' action, using 'page' query parameter
    else if (action === 'ongoing') {
      result = await Drakorindo.ongoing(page || "1"); // Default to page 1 if no page is specified
    }
    // Handle 'detail' action, using 'url' query parameter
    else if (action === 'detail' && url) {
      result = await Drakorindo.detail(url);
    } else {
      return res.status(400).json({ error: 'Invalid action or missing query parameter' });
    }
