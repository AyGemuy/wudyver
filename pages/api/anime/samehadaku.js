...
class Samehadaku {
  ...
  async search(query) {
    try {
      const res = await this.client.get(`${this.BASE_URL}/?${new URLSearchParams({ s: encodeURIComponent(query) })}`);
  ...
  async release() {
    try {
      ...
      for (let day of Object.keys(data)) {
        const res = await this.client.get(`${this.BASE_URL}/wp-json/custom/v1/all-schedule?${new URLSearchParams({ perpage: "20", day, type: "schtml" })}`);
        data[day] = res.data;
      }
      return data;
    } catch (err) {
...
export default async function handler(req, res) {
  try {
    ...
    const { query, action, url } = method === "GET" ? req.query : req.body;
