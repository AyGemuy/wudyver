...
export default async function handler(req, res) {
  ...
  try {
    const response = await fetch(
      `https://raw.githubusercontent.com/AyGemuy/RESTAPI/master/data/${text}.json`
    );

    if (!response.ok) {
      return res.status(404).json({ error: `Data untuk '${text}' tidak ditemukan.` });
    }
