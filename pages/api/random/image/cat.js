...
export default async function handler(req, res) {
  const { text, size, color, type, width, filter } = req.query;
  ...
  try {
    ...
    if (contentType.includes("image")) {
      const buffer = await response.arrayBuffer(); // Menggunakan arrayBuffer untuk gambar
      const uint8Array = new Uint8Array(buffer);
      res.setHeader("Content-Type", contentType);
      return res.send(Buffer.from(uint8Array)); // Mengirim gambar sebagai Buffer
    ...
    res.status(500).json({ error: 'Error fetching from Cataas API' });
  } finally {
    console.log(message);
  }
}
