...
class VoiceAPI {
  async createVoice(voiceId, url) {
    try {
      ...
      const { ext, mime } = (await fileTypeFromBuffer(Buffer.from(inputBuffer))) || {};
      ...
      const response = await fetch(
        'https://relikt-sweng465.vercel.app/api/voice/create_vtv',
        {
          method: 'POST',
          body: formdata,
        },
  ...
  async getModelData(type) {
    ...
    try {
      if (type === 'eleven') {
        ...
        const response = await fetch(
          'https://relikt-sweng465.vercel.app/api/voice/get_vtv_models',
        );
  ...
  async createTTS(voiceId, text) {
    try {
      const response = await fetch(
        'https://relikt-sweng465.vercel.app/api/voice/create_tts',
        {
          method: 'POST',
...
export default async function handler(req, res) {
  if (req.method === 'GET') {
    const { action, url, type, id: voiceId, text } = req.query;
