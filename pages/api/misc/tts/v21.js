...
class AIVoice {
  constructor() {
    ...
    this.upUrl = "https://i.supa.codes/api/upload";
  ...
  async uA(buf, mM) {
    console.log("[uA] Mengunggah audio ke server upload.");
    try {
      const fD = new FormData();
      const fE = mM.split("/")[1] || "mp3";
      const fN = `audio.${fE}`;
      ...
      const {
        data: uRes
      } = await axios.post(this.upUrl, fD, {
        headers: {
          ...fD.headers
        }
      });
      if (!uRes || !uRes.link) {
        throw new Error("Respons unggahan tidak valid atau link tidak ditemukan.");
      }
      console.log("[uA] Audio berhasil diunggah. URL:", uRes.link);
      return uRes.link;
    } catch (error) {
      console.error("[uA] Error mengunggah audio:", error.message);
      throw new Error("Error mengunggah audio: " + error.message);
    }
  }
  async tts({
    voice: vName = "Abigail",
