...
class XGroovy {
  ...
  async search(query = '') {
    if (!query) return { result: 'Query cannot be empty' };
    try {
      const { data } = await axios.get(randomProxyUrl + `${this.baseUrl}/search/${encodeURIComponent(query)}/`);
      ...
      return { result: 'Failed to fetch search data' };
    }
  }

  async detail(url = '') {
    if (!url) return { result: 'URL cannot be empty' };
    try {
      const { data } = await axios.get(randomProxyUrl + url);
      const $ = cheerio.load(data);
      return {
        result: {
      ...
      return { result: 'Failed to fetch detail data' };
    }
  }

  async photoai() {
    try {
      const { data } = await axios.get(randomProxyUrl + `${this.baseUrl}/photos/categories/ai/`);
      const $ = cheerio.load(data);
      return {
      ...
      return { result: 'Error fetching AI photos' };
    }
  }

  async photo() {
    try {
      const { data } = await axios.get(randomProxyUrl + `${this.baseUrl}/photos/`);
      const $ = cheerio.load(data);
      return {
      ...
      return { result: 'Failed to fetch photo albums' };
    }
  }
}
...
export default async function handler(req, res) {
  ...
  const { action = 'search', query: searchQuery = 'furry', url } = query;
  ...
  if (method === 'GET') {
    ...
    const actionResult = actions[action] ? await actions[action]() : { result: 'Invalid action' };
