...
class EromeAPI {
  ...
  async search(query) {
    ...
    try {
      ...
      if (albums.length === 0) {
        return { message: 'No albums found for the query' };
      }
      
      return albums;
    } catch (error) {
      console.error('Error fetching search results:', error);
      return { message: 'Error fetching search results. Please try again later.' };
    }
  }
  ...
  async detail(url) {
    try {
      ...
      if (!albumDetails.title) {
        return { message: 'Album details could not be fetched. Please check the URL.' };
      }

      return albumDetails;
    } catch (error) {
      console.error('Error fetching album details:', error);
      return { message: 'Error fetching album details. Please try again later.' };
    }
  }
}
