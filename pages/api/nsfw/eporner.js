...
class EpornerAPI {
  async search(q = 'indonesia') {
    try {
      ...
      return $("#vidresults .mb")
        .map((index, el) => ({
          id: $(el).data("id") || "ID not available",
          quality:
            $(el).find(".mvhdico span").text() || "Quality not available",
          ...
          uploader:
            $(el).find(".mb-uploader a").text() || "Uploader not available",
          link:
            new URL(
              $(el).find(".mbtit a").attr("href"),
              "https://www.eporner.com",
            ).href || "Link not available",
          thumbnail:
            $(el).find(".mbimg img").attr("src") || "Thumbnail not available",
        }))
        .get();
    } catch (error) {
  ...
  async detail(url) {
    try {
      ...
      return {
        ...
        thumbnail: thumbnail,
        download: $(".dloaddivcol .download-h264 a")
          .map((idx, downloadEl) => {
            const qualityMatch = $(downloadEl).text().match(/\d+p/),
              fileSizeMatch = $(downloadEl)
                .text()
                .match(/\d+\.\d+\s*MB/),
...
export default async function handler(req, res) {
  ...
  if (action === 'search' && query) {
    ...
    if (!action) {
      return res.status(400).json({ message: 'Parameter "action" tidak ditemukan. Harap masukkan "search" atau "detail".' });
    } else if (action === 'search' && !query) {
      return res.status(400).json({ message: 'Parameter "query" diperlukan untuk pencarian. Harap masukkan kata kunci pencarian.' });
    } else if (action === 'detail' && !url) {
      return res.status(400).json({ message: 'Parameter "url" diperlukan untuk detail. Harap masukkan URL yang valid.' });
    } else {
      return res.status(400).json({ message: 'Aksi yang tidak valid atau parameter yang hilang.' });
    }
  }
}
