...
class WordizeConverter {
  ...
  async uploadFile(html) {
    try {
      ...
      const {
        data
      } = await axios.post(`${this.baseURL}/UploadForThumbnails?SessionId=${this.sessionId}`, formData, {
        headers: headers
      });
      if (!data.length) throw new Error("Gagal mengupload file.");
      return {
        fileId: data[0],
        fileName: fileName,
        html: html
  ...
  async convertHTMLToImage({
    ...
    try {
      ...
      const upload = await this.uploadFile(html);
      if (!upload) return null;
      console.log("Mengonversi ke", output, "...");
