...
class IlovePDF {
  constructor(publicKey = 'project_public_c14b91af1ac4799b1f229e682d664154_x5xfs2fff307d4bde7d8b8b6bef3a0f2a9391') {
  ...
  async process(mediaBuffer, tool, payload) {
    try {
      ...
      const processResult = await this.processFile(server, task, serverFilename, tool, payload);
      const fileBuffer = await this.downloadFile(server, task);
      return { processResult, fileBuffer };
    } catch (e) {
...
const ilovePdf = new IlovePDF();
  if (req.method === 'POST') {
    // Handle POST request (input buffer)
    try {
      const { tool, payload } = req.body;
      const mediaBuffer = req.body.file; // Assuming the file is sent as a buffer
      if (!mediaBuffer || !tool) {
        return res.status(400).json({ error: 'Missing required parameters' });
      }

      const { processResult, fileBuffer } = await ilovePdf.process(mediaBuffer, tool, payload);
    ...
    // Handle GET request (input URL and params)
    try {
      const { tool, url, ...payload } = req.query;
      if (!url || !tool) {
        return res.status(400).json({ error: 'Missing required parameters' });
      }
