...
const imgGen = new ImgGen();
  try {
    const { type, ...options } = req.method === "POST" ? req.body : req.query;

    if (!type) return res.status(400).json({ error: "Type parameter is required" });

    if (req.method === "POST") {
      if (!req.body || !req.body.file)
        return res.status(400).json({ error: "Invalid input: Buffer expected" });

      const buffer = Buffer.from(req.body.file);
      const fileType = await fileTypeFromBuffer(buffer);

      if (!fileType || !fileType.mime.startsWith("image/"))
        return res.status(400).json({ error: "Invalid file type: Image required" });
      ...
      if (!response.ok)
        return res.status(400).json({ error: "Unable to fetch image from URL" });
