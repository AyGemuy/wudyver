...
async function dl_v1(url) {
  ...
  try {
    ...
    const filename =
      data.match(/<h1 class="intro">(.*?)<\/h1>/s)?.[1] || "unknown";
    const mimetype =
      data.match(/<div class="list">.*? - (.*?)<\/div>/)?.[1] || "";
    const downloadUrl = data.match(
    ...
    const { data: downloadPageData } = await axios.get(downloadUrl, {
      headers: headers,
    });
    const finalDownloadUrl = downloadPageData.match(
      /<a class="w3-button w3-blue w3-round" id="download" href="([^"]+)"/,
    )?.[1];
    ...
    const { data: fileBuffer, headers: fileHeaders } = await axios.get(
      finalUrl,
      {
        responseType: "arraybuffer",
        headers: {
    ...
    const filenameFinal =
      fileHeaders["content-disposition"]?.match(
        /filename=["']?([^"';]+)["']?/,
      )?.[1] || "unknown";
    return {
      status: true,
...
async function search_v1(id) {
  try {
    const { data } = await axios.get(
      `https://sfile-api.vercel.app/search/${id}`,
    );
    return data?.data;
  } catch (error) {
    throw error;
...
export default async function handler(req, res) {
  const { type, version = 1, query = '', page = 1, id = '', url = '' } = req.query;
  ...
  try {
    ...
    return res.status(400).json({ message: error.message || "Terjadi kesalahan" });
  }
}
