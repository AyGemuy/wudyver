...
async function createPaste(input) {
  const response = await fetch("https://api.paste.gg/v1/pastes", {
    ...
    body: JSON.stringify({
      files: [
        {
          content: { format: "text", value: input },
        },
      ],
    }),
...
export default async function handler(req, res) {
  if (req.method === 'POST' || req.method === 'GET') {
    const { input } = req.body || req.query;  // Mengambil input dari body untuk POST atau query untuk GET

    if (!input) {
      return res.status(400).json({ status: 1, error: "Input is required" });
    }
