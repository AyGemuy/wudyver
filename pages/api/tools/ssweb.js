...
const wudyScreenshot = async url => {
  try {
    const response = await fetch(`https://wudysoft-cookie.hf.space/screenshot?url=${url}`, {
      headers: {
        "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/123.0.0.0 Safari/537.36"
      }
    });
...
const Hexometer = async url => {
  try {
    ...
    const responseBuff = await fetch(`https://fullpagescreencapture.com/screen/${data.Property.liveScreenshot.hash}.jpg`, {
      headers: {
        "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/123.0.0.0 Safari/537.36"
      }
    });
    return Buffer.from(await responseBuff.arrayBuffer());
  } catch (e) {
    console.log(e);
  }
};
const Microlink = async url => {
  try {
...
const GeoBrowse = async (url, code = "us") => {
  try {
    const payload = {
      code: code,
      url: url
    };
    const response = await axios.post("https://us-central1-geotargetly-1a441.cloudfunctions.net/free-tool_geobrowse-gen-screenshots", payload, {
      headers: {
        "Content-Type": "application/json",
...
class ScreenshotProvider {
  ...
  async EuCentral(link) {
    try {
      const imageResponse = await EuCentral(link);
      return imageResponse;
    } catch (e) {
      console.log(e);
      throw e;
  ...
  async Hexometer(link) {
    try {
      const imageResponse = await Hexometer(link);
      return imageResponse;
    } catch (e) {
      console.log(e);
      throw e;
  ...
  async Microlink(link) {
    try {
      const imageResponse = await Microlink(link);
      return imageResponse;
    } catch (e) {
      console.log(e);
      throw e;
...
const ssweb = async (link, index = 1) => {
  const provider = new ScreenshotProvider();
  const methods = [provider.GoogleApis, provider.Hexometer, provider.Pikwy, provider.Microlink, provider.EuCentral, provider.apiFlash, provider.backup15, provider.pptr, provider.sShot, provider.screenshotLayer, provider.screenshotMachine, provider.shotsnap, provider.thumIO, provider.urlbox, provider.webshotElzinko, provider.webss, provider.wudyScreenshot, provider.GeoBrowse];
  if (index < 1 || index > methods.length) throw new Error("Provider tidak ditemukan");
  const selectedMethod = methods[index - 1];
  try {
    return await selectedMethod(link);
  } catch (error) {
    console.error(`Error pada metode terpilih: ${error}`);
    for (const method of methods) {
      try {
        return await method(link);
      } catch (error) {
        console.error(`Error pada metode ${method.name}: ${error}`);
      }
    }
...
export default async function handler(req, res) {
  ...
  try {
    const result = await ssweb(url, v);
