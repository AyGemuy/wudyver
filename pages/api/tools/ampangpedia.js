...
class AmpangPedia {
  ...
  async post(apiUrl, opts = {}) {
    try {
      ...
      return e?.response || { error: "Request failed" };
    }
  }
  ...
  async prepaid() {
    return {
      order: async (service, id, server) =>
        service && id
          ? this.post(this.api.prepaid, {
              type: "order",
              service,
              data_no: `${id}${server || ""}`,
            })
          : { status: false, message: `Missing ${service ? "data_no" : "service code"}` },
      status: async (trxid, limit) =>
        this.post(this.api.prepaid, {
          type: "status",
          trxid: trxid || "",
          limit: typeof limit === "number" ? limit : "",
        }),
      services: async (filter_type, filter_value) =>
        this.post(this.api.prepaid, {
          type: "services",
  ...
  async media() {
    return {
      order: async (service, id, server) =>
        service && id
          ? this.post(this.api.social_media, {
              type: "order",
              service,
              data_no: `${id}${server || ""}`,
            })
...
export default async function handler(req, res) {
  const instance = new AmpangPedia(
    "4IBPmfPZ",
    "9WMugYrv57cppyyEAZmb0LVXcWagTjr6YGbFqBZd1WeHDq1tTxKLwc2R2t0l36GA"
  );

  try {
    const { type, action, params } = req.method === "POST" ? req.body : req.query;
