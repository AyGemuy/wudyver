...
export default async function handler(req, res) {
    ...
    const { code, lang, to } = req.body;

    if (!code || !lang || !to) {
        return res.status(400).json({ error: "Missing required fields: code, lang, or to" });
    }
    ...
    try {
        ...
        return res.status(500).json({ error: "Failed to convert code", details: error.message });
    }
}
