...
const asyncFunctions = _(
  ...
  if (req.method === 'GET') {
    ...
    return res.status(200).json({ providers: providers.length ? providers : Provider });
  }

  if (req.method === 'POST') {
    const { file, url: bodyUrl } = req.body;
    const { url: queryUrl, option = "all" } = req.query;

    const url = queryUrl || bodyUrl;

    if (!file && !url) {
      return res.status(400).json({ error: 'Either Media (file) or URL is required' });
    }
    ...
    try {
      ...
      for (const { name, func } of selectedProviders) {
        try {
          const uploadResponse = await func(fileBuffer);
          if (uploadResponse) {
            spinner.succeed(chalk.green(`Upload successful with ${name}`));
            return res.status(200).json({ result: uploadResponse });
          } else {
