...
export default async function handler(req, res) {
  ...
  const asyncFunctions = _(Object.getOwnPropertyNames(Object.getPrototypeOf(upload)))
  ...
  if (req.method === 'GET') {
    ...
    return res.status(200).json({ providers: providers.length ? providers : ['No providers available'] });
  }

  if (req.method === 'POST') {
    const file = req.body;
    const { option = 'all' } = req.query;
    if (!file) {
    ...
    const { formData } = await createFormData(fileBuffer, 'file');
