...
export default async function handler(req, res) {
  ...
  if (req.method === "GET") {
    const allHost = Object.keys(asyncFunctions);
    return res.status(200).json({
      allHost: allHost.length ? allHost : ["No allHost available"]
    });
  }
  if (req.method === "POST") {
    try {
      ...
      try {
        const allHost = Object.entries(asyncFunctions).map(([name, func]) => ({
          name: name,
          func: func
        }));
        const selectedHost = getHost(host, allHost);
        for (const {
            name,
            func
          }
          of selectedHost) {
          try {
            const result = await func(buffer);
            if (result) {
        ...
        throw new Error("All allHost failed to upload.");
      } catch (error) {
        spinner.stop();
...
function getHost(host, allHost) {
  if (host === "all") {
    return allHost;
  }
  const index = Number(host);
  if (_.isNumber(index) && _.inRange(index, 1, allHost.length + 1)) {
    return [allHost[index - 1]];
  }
  throw new Error(`Invalid input. Please provide 'all' or a valid index.\nAvailable Indexes:\n${allHost.map((p, i) => `${i + 1}: ${p.name}`).join("\n")}`);
}