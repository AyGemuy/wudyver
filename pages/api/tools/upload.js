...
class Uploader {
    ...
    try {
      const { formData } = await createFormData(content, "file"),
        response = await fetch(
          "https://changyan.sohu.com/api/2/comment/attachment",
          {
            method: "POST",
            body: formData,
            headers: {
  ...
  async Pomf2(content) {
    ...
    try {
      ...
      return (
        spinner.succeed(chalk.green("Uploaded to Pomf2")), json.files[0]?.url
      );
    } catch (error) {
      handleErrorResponse(error, spinner);
    }
  ...
  async Gizai(content) {
    ...
    try {
      const { formData } = await createFormData(content, "file"),
        response = await fetch("https://app.giz.ai/api/tempFiles", {
          method: "POST",
          body: formData,
          headers: {
            "User-Agent": fakeUserAgent(),
  ...
  async Ucarecdn(content) {
    ...
    try {
      const { formData, ext } = await createFormData(content, "file");
      ...
      return (
        ...
        `https://ucarecdn.com/${file}/${generateSlug}.${ext || "bin"}`
      );
    } catch (error) {
      handleErrorResponse(error, spinner);
    }
  ...
  async FreeImage(content) {
    ...
    try {
      ...
      return (
        ...
        response.data?.image.url || response.data?.image?.image.url
      );
    } catch (error) {
      handleErrorResponse(error, spinner);
    }
  ...
  async Babup(content) {
    ...
    try {
      const { formData } = await createFormData(content, "file_1_");
      ...
      return (
        spinner.succeed(chalk.green("Uploaded to Babup")), id ? result : output
      );
    } catch (error) {
      handleErrorResponse(error, spinner);
    }
  ...
  async Tmpfiles(content) {
    ...
    try {
      ...
      return originalURL
        ? `https://tmpfiles.org/dl/${originalURL.split("/").slice(-2).join("/")}`
        : null;
    } catch (error) {
      handleErrorResponse(error, spinner);
    }
  ...
  async Kraken(content) {
    ...
    try {
      const { data } = await axios.get(referer),
        uploadUrl = data?.match(uloadUrlRegexStr)?.[1];
  ...
  async Filebin(content) {
    ...
    try {
      const { formData, ext } = await createFormData(content, "file"),
        uploadURL = `https://filebin.net/${(await fetch("https://filebin.net/").then((res) => res.text())).match(/var\s+bin\s*=\s*['"]([^'"]+)['"]/)?.[1]}/${generateSlug}.${ext || "bin"}`,
  ...
  async Fexnet(content) {
    ...
    try {
      ...
      const response = await fetch(
        "https://fexnet.zendesk.com/api/v2/uploads.json",
        {
          method: "POST",
          body: formData,
          headers: {
    ...
    try {
      ...
      const response = await fetch(
        (
          await (
            await fetch(
              "https://doodapi.com/api/upload/server?key=" +
                (key || "13527p8pcv54of4yjeryk"),
            )
          ).json()
        ).result,
        {
  ...
  async Gozic(content) {
    ...
    try {
      ...
      const { url: result } = await response.json();
      return result;
    } catch (error) {
      handleErrorResponse(error, spinner);
    }
  ...
  async UploadEE(content) {
    ...
    try {
      const baseUrl = "https://www.upload.ee",
        response = await fetch(
          `${baseUrl}/ubr_link_upload.php?rnd_id=${Date.now()}`,
        );
      if (!response.ok) throw new Error("Failed to get upload link");
      const uploadId = ((await response.text()).match(/startUpload\("(.+?)"/) ||
        [])[1];
  ...
  async Uploadify(content) {
    ...
    try {
      const { formData } = await createFormData(content, "files[]"),
        response = await fetch(
          "https://uploadify.net/core/page/ajax/file_upload_handler.ajax.php?r=uploadify.net&p=https&csaKey1=1af7f41511fe40833ff1aa0505ace436a09dcb7e6e35788aaad2ef29d0331596&csaKey2=256b861c64ec1e4d1007eb16c68b3cfc5cb8170658b1053b7185653640bb3909",
          {
            method: "POST",
            body: formData,
            headers: {
  ...
  async Sojib(content) {
    ...
    try {
      const { formData } = await createFormData(content, "file"),
        response = await fetch("https://chat-gpt.photos/api/uploadImage", {
          method: "POST",
          body: formData,
          headers: {
            "User-Agent": fakeUserAgent(),
  ...
  async Knowee(content) {
    ...
    try {
      const { formData } = await createFormData(content, "files"),
        response = await fetch(
          "https://core.knowee.ai/api/databank/pub-files",
          {
            method: "POST",
            body: formData,
            headers: {
              ...
              Referer:
                "https://knowee.ai/webapp/homework/1b7562c3-e9ab-442d-9c00-4b0eea2310e3",
            },
          },
        );
...
const asyncFunctions = _(
  Object.getOwnPropertyNames(Object.getPrototypeOf(upload)),
)
  .filter((prop) => typeof upload[prop] === "function" && prop !== "constructor")
  .map((func) => [func, upload[func].bind(upload)])
  .fromPairs()
  .value();

export default async function handler(req, res) {
  if (req.method === 'GET') {
    // If GET request, return available providers
    const providers = Object.entries(asyncFunctions).map(([name]) => name);
    return res.status(200).json({ providers: providers ? providers : Provider });
  }

  if (req.method === 'POST') {
    const inp = req.body; // Get 'inp' directly from req.body
    const { option = "all" } = req.query; // Get 'option' from query params

    if (!inp) {
      return res.status(400).json({ error: 'Media (inp) is required' });
    }
    ...
    try {
      ...
      const selectedProviders =
        option === "all"
          ? providers
          : _.isNumber(Number(option)) && _.inRange(Number(option), 1, providers.length + 1)
          ? [providers[Number(option) - 1]]
          : (() => {
              throw new Error(
                `Invalid input. Please provide 'all' or a valid index.\nAvailable Indexes:\n${providers.map((p, i) => `${i + 1}: ${p.name}`).join("\n")}`
              );
            })();

      for (const { name, func } of selectedProviders) {
        try {
          const result = await func(Buffer.from(inp));
          if (result) {
      ...
      spinner.stop();
      return res.status(500).json({ error: `Error during upload: ${error.message}` });
    }
  } else {
