...
const getInstagramUserID = async (username) => {
  try {
    const response = await axios.post('https://ig.informatikamu.id/fitur/followers-instagram-gratis?formType=findUserID', 
      new URLSearchParams({
        username: username
      }), 
      {
...
export default async function handler(req, res) {
  ...
  try {
    ...
    console.error(error);
    return res.status(500).json({ error: 'Failed to fetch Instagram user ID' });
  }
}
