// pages/api/dynamic.js
import axios from 'axios';

export default async function handler(req, res) {
  const { api = 'api.paxsenix.biz.id', path = '/ai/gemini', method = 'get', ...queryParams } = req.query;

  const allowedMethods = ['GET', 'POST'];
  const requestMethod = method.toUpperCase();
  
  if (!allowedMethods.includes(requestMethod)) {
    return res.status(400).json({ error: 'Invalid HTTP method. Only GET and POST are allowed.' });
  }

  if (!path || !requestMethod) {
    return res.status(400).json({ error: 'Missing required parameters: path or method.' });
  }
  ...
  try {
    ...
    return res.json({ result: apiRes.data });
  } catch (error) {
