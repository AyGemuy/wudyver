...
const checkWebsite = async (websiteUrl) => {
  ...
  try {
    const { data } = await axios.get(apiUrl.toString(), { headers: DEFAULT_HEADERS, maxRedirects: 5 });
    const domainName = websiteUrl.replace(/^(https?:\/\/)?/, '');
    const { isDown, lastChecked } = data;
    let lastCheckedDate = new Date(lastChecked * 1000);

    if (isNaN(lastCheckedDate.getTime())) lastCheckedDate = new Date();

    const statusMessage = isDown 
      ? `Website ${domainName} tidak bisa diakses. Mungkin sedang mengalami masalah.` 
      : `Website ${domainName} dapat diakses dengan baik.`;
    ...
    const secondsAgo = Math.floor((Date.now() - lastCheckedDate.getTime()) / 1000);
    const relativeTime = new Intl.RelativeTimeFormat('id', { numeric: 'auto' }).format(-secondsAgo, 'second');
...
export default async function handler(req, res) {
  try {
    ...
    if (!link) {
      return res.status(400).json({ error: 'Parameter "url" harus diisi untuk memeriksa status website.' });
    }

    const websiteStatus = await checkWebsite(link);
    res.status(200).json({ success: true, data: websiteStatus });
  } catch (error) {
