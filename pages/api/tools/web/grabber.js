...
export default async function handler(req, res) {
    ...
    if (!link) {
        return res.status(400).json({ error: 'Link tidak disediakan.' });
    }
    ...
    if (!domain.test(new URL(link).hostname)) {
        return res.status(400).json({ error: 'Domain tidak diizinkan untuk di-grab.' });
    }
    ...
    try {
        const response = await fetch('https://okdebrid.com/api?mode=plg&token=__', {
            ...
            body: new URLSearchParams({ link: link, lang: 'en-US', chck: '.' })
        });
        ...
        if (data.link) {
            ...
            if (result) {
                ...
                return res.status(500).json({ error: 'Dekode gagal.', data });
            }
        } else {
            return res.status(400).json({ error: 'Grabber link tidak ditemukan.' });
        }
    } catch (error) {
        console.error(error);
