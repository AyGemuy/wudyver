  ...
  try {
    ...
    while (attempts < 10) {
      ...
      if (statusResult.isFinished) {
        ...
        return { name: `${md5}.zip`, media: buffer.toString('base64'), link: `${apiUrl}/api/downloadArchive/${md5}` };
      }
...
export default async function handler(req, res) {
  const { link, renameAssets = false, saveStructure = false, alternativeAlgorithm = false, mobileVersion = false } = req.query;
