...
async function createPaste({ title = '', content }) {
  ...
  const { data: result } = await axios.post(API_URL, data, {
    headers: { "Content-Type": "application/x-www-form-urlencoded" },
  });

  const rawUrl = result.replace(/^(https:\/\/pastebin\.com\/)([a-zA-Z0-9]+)$/, "$1raw/$2");

  return { status: result ? 0 : 1, original: result, raw: rawUrl };
}

export default async function handler(req, res) {
  if (req.method === 'POST' || req.method === 'GET') {
    const { title, content } = req.method === 'POST' ? req.body : req.query;
