...
async function convert(fields) {
  if ("string" == typeof fields && "list" === fields?.toLowerCase())
  ...
  if (fields?.file) {
    if (!fields.filename)
      throw new Error(
        "filename must be provided to upload files.(with extension)",
      );
  ...
  if (
    (type.req_params &&
      type.req_params.forEach((e) => {
        if (!org_keys.includes(e))
          throw new Error(`"${e}" is a required param.`);
      }),
    ...
    if (
      ...
      throw new Error(
        `Either one of these params has to be provided: ${type.either_params.join(", ")}`,
      );
  }
  let link = await axios({
      ...
      throw error.response
            ...
            JSON.stringify(
              {
                ...
                data: error.response.data.length
                  ? error.response.data
        ...
        : new Error("Oops, something unknown happened! :(");
    }),
  ...
  if (img_url.includes("undefined"))
    throw new Error(
      "Something unknown happened here... please report to the creator",
    );
  return img_url;
}
...
async function render(fields) {
  ...
  for (let i = 0; i < fields.files.length; i++) {
    if (!fields.files[i].data)
      throw new Error(`File buffer not provided for files[${i}]`);
    if (!fields.files[i].name)
      throw new Error(`File name not provided for files[${i}]`);
...
const validTypes = [
  "apng-avif", "apng-gif", "apng-mp4", "apng-webp", "avif-gif", "avif-jpg", "avif-png", 
  "bmp-gif", "bmp-jpg", "bmp-png", "gif-apng", "gif-avif", "gif-jpg", "gif-mp4", "gif-png", 
  "gif-sprite", "gif-webp", "jpg-avif", "jpg-webp", "mng-apng", "png-bmp", "png-gif", "png-jpg", 
  "png-webp", "sprite-img", "sprite-imgage", "svg-jpg", "svg-png", "video-apng", "video-avif", 
  "video-gif", "video-jpg", "video-png", "video-webp", "webp-avif", "webp-gif", "webp-jpg", 
  "webp-mp4", "webp-png"
];

export default async function handler(req, res) {
  const { type = "png-jpg", url = "https://malik-jmk.us.kg/favicon.png", option = "convert", delay = 20, files = [] } = req.method === "GET" ? req.query : req.body;
  ...
  if (option === "convert") {
    ...
    try {
      const result = await convert({ type, url });
      return res.status(200).json(result instanceof Object ? result : { result });
    } catch (error) {
      console.error(error);
  ...
  if (option === "render") {
    try {
      const fields = { delay, files: JSON.parse(files) };
  ...
  if (option === "overlay") {
    try {
      const fields = { file: req.files?.file, filename: req.files?.filename };
