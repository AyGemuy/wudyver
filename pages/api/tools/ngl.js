...
export default async function handler(req, res) {
  ...
  const { link, message, type = 'anonymous' } = req.query;

  if (!link || !message) {
    return res.status(400).json({ error: 'Link and message are required' });
  }

  try {
    const success = await ngl.send(link, message, type);
    if (success) {
      return res.status(200).json({ message: `Pesan (${type}) berhasil dikirim!` });
    } else {
      return res.status(500).json({ error: `Pesan (${type}) gagal dikirim` });
    }
  } catch (error) {
    return res.status(500).json({ error: 'Terjadi kesalahan saat mengirim pesan', details: error.message });
  }
}
