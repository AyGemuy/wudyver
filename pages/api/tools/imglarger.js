...
class ImgLarger {
  ...
  async uploadImage(input, scaleRadio = 2, isLogin = 0) {
    ...
    if (typeof input === "string") {
      ...
      formData.append("myfile", blob, { filename: "uploaded_image.jpg" });
  ...
  async checkStatus(uploadResponse, scaleRadio, isLogin) {
    ...
    const response = await fetch(`${this.baseURL}/CheckStatus`, {
      ...
      body: JSON.stringify({ code, scaleRadio, isLogin }),
    });
...
const imgLarger = new ImgLarger();
  try {
    const { input, scaleRadio = 2, isLogin = 0 } = req.method === "POST" ? req.body : req.query;
