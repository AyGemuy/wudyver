...
const fdroid = {
  search: async (query) => {
    const { data: html } = await axios.get(`https://search.f-droid.org/?q=${encodeURIComponent(query)}&lang=en`);
  ...
  package: async (url) => {
    ...
    return { name: packname, summary: packsuma, description: packdesc, links: packlinks, versions };
  }
};
