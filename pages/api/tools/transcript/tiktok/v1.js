...
class TokAudit {
  ...
  async fetchVideoData(link, getTranscript = false) {
    ...
    try {
      const { data } = await axios.get(`${this.api.base}${this.api.fetch}`, {
        params: { video: link, get_transcript: getTranscript, ip: await this.getIP() },
        headers: this.headers,
      });

      if (!data?.data) return this.error(404, "Video not found.");

      return { status: true, code: 200, result: this.formatData(data.data, getTranscript ? data.subtitles : false) };
    } catch (e) {
      return this.error(e.response?.status || 500, "Server error.");
    }
  }

  formatData({ id, desc, createTime, video, author, stats, music, challenges }, subtitles) {
    return {
      video: { id, desc, createTime, duration: video.duration, url: video.downloadAddr, quality: video.videoQuality },
      author: { id: author.id, name: author.nickname, avatar: author.avatarLarger, verified: author.verified },
      stats: { likes: stats.diggCount, shares: stats.shareCount, comments: stats.commentCount, views: stats.playCount },
      music: { id: music.id, title: music.title, url: music.playUrl, author: music.authorName },
      challenges: challenges.map(({ id, title }) => ({ id, title })),
      subtitles,
    };
  }

  error(code, message) {
    return { status: false, code, result: { message } };
  }
}
