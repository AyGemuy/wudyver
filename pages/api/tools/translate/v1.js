...
export default async function handler(req, res) {
  if (req.method === "POST" || req.method === "GET") {
    const { text, target = "ID", source = "auto" } =
      req.method === "POST" ? req.body : req.query;

    if (!text) {
      return res.status(400).json({ status: 1, error: "Text is required" });
    }
    ...
    try {
      ...
      if (data.result?.translations?.[0]?.beams) {
        const translations = data.result.translations[0].beams.map(
          (beam) => beam.sentences.map((sentence) => sentence.text).join(" ")
        );
