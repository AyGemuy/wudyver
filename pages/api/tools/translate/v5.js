...
class Transcat {
  constructor() {
    this.languages = {
      ...
      'ne': 'Nepali', 'no': 'Norwegian', 'ny': 'Nyanja (Chichewa)', 'ps': 'Pashto', 'fa': 'Persian',
  ...
  async check(text, sourceLang = 'auto', targetLang = 'en') {
    ...
    try {
      ...
      const { token, cookies } = await tokens();

      const response = await axios.post('https://www.translatecat.com/', 
        `content=${encodeURIComponent(text)}&source=${sourceLang}&target=${targetLang}&_token=${token}`,
        {
          headers: {
...
export default async function handler(req, res) {
  const { action, text, sourceLang, targetLang } = req.query;
  ...
  try {
    ...
    switch (action) {
      case 'check':
        if (!text) {
          return res.status(400).json({ success: false, error: 'Teks diperlukan untuk terjemahan.' });
        }
      ...
      case 'langs':
        ...
        return res.status(200).json({ success: true, langs });
