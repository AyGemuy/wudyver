...
class ShortLink {
  ...
  async gen(url, index = 0) {
    ...
    if (index < 0 || index >= providers.length) {
      console.error("Invalid index. Please choose a valid provider index.");
      return null;
    }

    try {
      return await providers[index].call(this, url);
    } catch (error) {
      console.error(error);
      return null;
    }
...
const shortlink = async (url, index) => {
  return await shortLink.gen(url, index);
}
export default async function handler(req, res) {
  const { url, type } = req.query;

  if (!(url || type)) return res.status(400).json({ message: "No url, type provided" });

  const result = await shortlink(url, type);
