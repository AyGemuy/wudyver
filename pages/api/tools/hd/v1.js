...
const processImage = async (imageUrl, scale) => {
  ...
  const blob = new Blob([fileData], { type: fileType?.mime || 'image/jpeg' });
  ...
  const progressRes = await request('status', { code: uploadRes.token });
...
export default async function handler(req, res) {
  try {
    ...
    if (!imageUrl || !scale) {
      return res.status(400).json({ error: 'Missing required parameters: imageUrl or scale' });
    }
