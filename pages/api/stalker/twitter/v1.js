...
export default async function handler(req, res) {
  ...
  try {
    ...
    const result = {
      pp_user: $("div.relative.w-full.h-full.rounded-full.cursor-pointer.profile-image > img")
        .attr("src") ?? null,
      ...
      description: $("div.p-3.border-t.border-gray-200 > p")
        .text()
        .trim()
        .replace(/\n/g, "") || 'No description',
      ...Object.fromEntries(
        $("div.flex.justify-center > div.px-4").map((idx, el) => [
          $(el)
            .find("div.text-xs.font-bold.text-center.text-gray-600.uppercase")
            .text()
            .toLowerCase(),
