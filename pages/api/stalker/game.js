...
export default async function handler(req, res) {
  ...
  const { provider, userId: id, zoneId } = query;
  ...
  if (!provider || !id) {
    return res.status(400).json({ result: null, error: 'provider dan userId harus disediakan' });
  }
  ...
  switch (providerLowercase) {
    ...
    case 'codmobile':
      ...
      break;
    default:
      return res.status(400).json({ result: null, error: 'Game tidak dikenali' });
  }
  ...
  try {
    ...
    if (response.ok) {
      ...
      return res.status(400).json({ result: null, error: 'Gagal mendapatkan data.' });
    }
  } catch (error) {
    return res.status(500).json({ result: null, error: 'Terjadi kesalahan saat mengambil data.' });
  }
}
