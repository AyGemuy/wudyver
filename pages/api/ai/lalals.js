...
const lalal = async (input_text, input_tags) => {
  return await lalals.createMusic(input_text, input_tags);
};
...
export default async function handler(req, res) {
  ...
  if (!(prompt || tags))
    return res.status(400).json({ message: "No prompt, tags provided" });

  const result = await lalal(prompt, tags);
