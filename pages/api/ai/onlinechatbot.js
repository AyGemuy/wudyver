...
export default async function handler(req, res) {
  const chatbot = {
    getCharacters: async () => {
      try {
        const response = await axios.get('https://malik-jmk.us.kg/json/ai/system-message.json');
  ...
  const { query } = req;
  const { action, characterName, prompt, userQuestion } = query;

  if (!action) {
    return res.status(400).json({ message: 'Action parameter tidak ditemukan.' });
  }
