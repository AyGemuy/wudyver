  ...
  async quiz(topic, num = 1) {
    try {
      ...
      for (let i = 0; i < num; i++) {
        ...
        const soal = questionData.question;
        const jawaban = questionData.answer;
        const alternatives = questionData.alternatives;
        const shuffled = [...jawaban, ...alternatives].sort(() => Math.random() - 0.5);

        const alphabet = 'ABCDEFGHIJKLMNOPQRSTUVWXYZ'.split('');
        const quiz = {
          soal: soal,
          jawaban: shuffled.map((option, index) => ({
            teks: option,
            huruf: alphabet[index] || '',
            benar: (String(option).toLowerCase() === String(jawaban).toLowerCase()) ? true : false
          })),
...
export default async function handler(req, res) {
  const { prompt, num } = req.method === "GET" ? req.query : req.body;
  ...
  try {
    ...
    res.status(500).json({ error: 'Error fetching quiz data', details: error.message });
  }
}
