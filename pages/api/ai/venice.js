...
export default async function handler(req, res) {
  ...
  const { type, prompt, ...customPayload } = req.query;
  ...
  try {
    ...
    if (type === "chat") {
      const chatPayload = {
        ...
        temperature: parseFloat(customPayload.temperature) || 0.8,
        topP: parseFloat(customPayload.topP) || 0.9,
      };
      ...
      const imagePayload = {
        ...
        favoriteImageStyles: customPayload.favoriteImageStyles
          ? JSON.parse(customPayload.favoriteImageStyles)
          : [],
