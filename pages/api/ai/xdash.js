...
export default async function handler(req, res) {
  ...
  const { query, search_uuid = "sniqIUPOkv8RIj6TWnB1j", visitor_uuid = "89072f2dde4f20c276ed5dd9242eaa4f", token = "U2FsdGVkX1994yT0p52bEy373unUukq49cSd9K7QMjQ=" } = req.query;

  if (!query)
    return res.status(400).json({ error: "Missing required parameter: query" });

  const payload = { query, search_uuid, visitor_uuid, token };
  const headers = {
