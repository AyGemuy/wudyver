...
class ChatSkyByte {
  ...
  async chat(prompt = 'Pak', systemMessage = "You are ChatGPT, a large language model trained by OpenAI. Follow the user's instructions carefully. Respond using markdown.", temperature = 0.8, top_p = 1) {
    const payload = {
      prompt,
      options: {},
...
export default async function handler(req, res) {
  ...
  const { prompt = 'Pak', systemMessage, temperature = 0.8, top_p = 1 } = req.query;

  const chatSkyByte = new ChatSkyByte();

  try {
    const response = await chatSkyByte.chat(
      prompt, 
      systemMessage ? systemMessage : "You are ChatGPT, a large language model trained by OpenAI. Follow the user's instructions carefully. Respond using markdown.",
      temperature, 
      top_p
