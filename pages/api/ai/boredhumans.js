...
class BoredHumans {
  ...
  async executeTool(tool, { prompt, ...options }) {
    try {
...
export default async function handler(req, res) {
  if (req.method === "GET") {
    const { action, ...options } = req.query;
    ...
    try {
      const boredHumans = new BoredHumans();

      if (!(action in boredHumans.tools)) {
        return res.status(400).json({ error: "Invalid action." });
      }

      const result = await boredHumans.executeTool(action, options);
