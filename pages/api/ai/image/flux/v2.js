...
const endpoints = [
  ...
  "generate_image_1",
  "realtime_generation",
  "realtime_generation_1",
  "realtime_generation_2",
  "realtime_generation_3",
];
...
const fetchResult = async (endpoint, prompt, width, height) => {
  try {
    ...
    if (!eventId) return { endpoint, status: "error", message: "Invalid Event ID" };

    let status = "pending", result = null;

    while (status === "pending") {
      const { data: pollResponse } = await axios.get(`${url}/${eventId}`);
      status = pollResponse.status || "pending";
      result = pollResponse.data || null;
      if (status === "pending") await new Promise((r) => setTimeout(r, 2000));
    }

    return status === "completed" ? { endpoint, status, result } : { endpoint, status: "error", message: result || "Unknown Error" };
  } catch (error) {
    return { endpoint, status: "error", message: error.message || "An unexpected error occurred" };
  }
};
...
export default async (req, res) => {
  try {
    ...
    if (type < 0 || type >= endpoints.length)
      return res.status(400).json({ message: "Invalid type. Must be between 1 and 7." });
