  ...
  create: async (query, ration = 2) => {
    ...
    try {
      const response = await axios.get(
        `https://1yjs1yldj7.execute-api.us-east-1.amazonaws.com/default/ai_image?prompt=${encodeURIComponent(query)}&aspect_ratio=${fluximg.ratios[ration - 1]}`,
        config
      );
...
export default async function handler(req, res) {
    const { query, ration } = req.method === 'GET' ? req.query : req.body;
    if (!query) {
    ...
    try {
      ...
      if (!imageBuffer) {
        return res.status(400).json({ error: 'Invalid ratio or other error' });
      }
