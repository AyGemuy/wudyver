...
class Aiease {
    ...
    async img2img(type, input, { style = 4, mode = 'general' } = {}) {
        if (!this.AUTH_TOKEN) await this.getToken();
        try {
...
export default async function handler(req, res) {
  const { type, input, options } = req.method === "GET" ? req.query : req.body;
  
  if (!type || !input) {
    return res.status(400).json({ 
      error: "Missing required fields", 
      required: { type: "txt2img | img2img", input: "string", options: "optional object" } 
    });
  }
  ...
  try {
    ...
    switch (type) {
      case "txt2img":
        result = await aiease.text2img(input, options);
        break;
      case "img2img":
        result = await aiease.img2img(type, input, options);
        break;
        case "style":
        result = await aiease.getStyle(input);
        break;
      default:
        return res.status(400).json({ 
          error: "Invalid type provided", 
          valid_types: ["txt2img", "img2img", "style"] 
        });
    }
