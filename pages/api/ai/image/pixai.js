...
export default async function handler(req, res) {
  if (req.method === 'GET') {
    const { text, image = 0, isNsfw = false } = req.query;
    ...
    try {
      ...
      if (!images || images.length === 0) {
        return res.status(404).json({ error: 'No results found for the given text.' });
      }

      const output = await apiResponse(images[image].node.id);
      if (!output || !output.media || !output.media.urls) {
        return res.status(500).json({ error: 'Error fetching artwork information.' });
      }
