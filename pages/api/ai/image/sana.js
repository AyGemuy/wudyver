...
export default async function handler(req, res) {
  if (req.method === 'GET') {
    const { prompt, weight, height, guiscale, paguiscale, nis, step, sid } = req.query;

    try {
      const response = await sanai.create(
        prompt || 'Example', parseInt(weight) || 1024, parseInt(height) || 1024, parseInt(guiscale) || 5,
        parseInt(paguiscale) || 2, parseInt(nis) || 18, parseInt(step) || 20, parseInt(sid) || -1
      );
