...
class Zaiwen {
  async chat({ prompt, model, id, key }) {
    const payload = {
  ...
  async draw({ prompt, model, ratio, seed }) {
    const payload = {
  ...
  async gemini({ prompt, system, key }) {
    const payload = {
      message: [
        { role: "system", content: system || "You are Ai" },
        { role: "user", content: prompt || "Yolo" }
      ],
      user_key: key || null,
    };
  ...
  async suno({ tags, title, prompt, model, instrumental }) {
    const payload = {
...
export default async function handler(req, res) {
  ...
  try {
    const { action, prompt, system, model, id, key, tags, title, instrumental } = req.query;

    let result;

    switch (action) {
      case 'chat':
        result = await zaiwen.chat({ prompt, model, id, key });
        break;
      case 'draw':
        result = await zaiwen.draw({ prompt, model, ratio: req.query.ratio, seed: req.query.seed });
        break;
      case 'bing':
        result = await zaiwen.bing({ prompt });
        break;
      case 'gemini':
        result = await zaiwen.gemini({ prompt, system, key });
        break;
      case 'suno':
        result = await zaiwen.suno({ tags, title, prompt, model, instrumental });
        break;
      default:
