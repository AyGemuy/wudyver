...
class PxPic {
  ...
  async hdImage(imageUrl) {
    ...
    try {
      const response = await axios.post(endpoint, { imageUrl }, { headers: { 'Content-Type': 'application/json' } });
  ...
  async addFilter(imageUrl, filterKey) {
    ...
    try {
      const response = await axios.post(endpoint, { imageUrl, filterKey: validFilterKey }, { headers: { 'Content-Type': 'application/json' } });
      return response.data.resultImageUrl;
    } catch (error) {
...
export default async function handler(req, res) {
  ...
  const { action, url, key } = req.query;
    ...
    switch (action) {
      ...
      case 'filter':
        if (!key || isNaN(key) || key < 1 || key > 70) {
          return res.status(400).json({ message: 'Key must be a number between 1 and 70' });
        }
