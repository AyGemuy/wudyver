...
class MyMapAI {
  constructor() {
    ...
    this.client = axios.create({
      headers: {
        ...
        cookie:
          "AWSALBAPP-0=_remove_; AWSALBAPP-1=_remove_; AWSALBAPP-2=_remove_; AWSALBAPP-3=_remove_; i18n_redirected=en; auth.strategy=graphql; auth._token.graphql=false; auth._refresh_token.graphql=false; _gcl_au=1.1.736721384.1741145008",
        origin: this.baseUrl,
        priority: "u=1, i",
        "sec-ch-ua":
  ...
  async chat({ prompt, system: systemMessage = "", id: boardId = null }) {
    try {
      ...
      const messages = [];
      if (systemMessage) messages.push({ type: "text", content: systemMessage });
      messages.push({ type: "text", content: prompt });
