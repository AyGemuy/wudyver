...
async function polybuzzAi(prompt, selectId = '98399308486') {
  ...
  try {
    const { data: response } = await axios.post('https://api.polybuzz.ai/api/conversation/msgbystream', formData, { headers });
...
export default async function handler(req, res) {
  const { prompt, selectId } = req.query;
  ...
  try {
    ...
    if (response) {
      ...
      return res.status(500).json({ error: 'Failed to retrieve data from PolyBuzz API' });
    }
  } catch (error) {
