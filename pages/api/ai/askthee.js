...
export default async function handler(req, res) {
  const { prompt, type = 3 } = req.query;

  if (!(prompt || type))
    return res.status(400).json({ message: "No prompt, type provided" });

  const result = await AskThee(prompt, type);
  return res.status(200).json({ result: { result: typeof result === "object" ? result : result } });
}
