...
export default async function handler(req, res) {
  ...
  const { action, query, page, pageSize, id, text,
    init_audio,
    language,
    emotion,
    speed } = req.query;

  const arting = new Arting();
  try {
    let result;

    switch (action) {
      case "voicemodel":
        result = await arting.voicemodel(query, parseInt(page) || 1, parseInt(pageSize) || 20);
        break;
      ...
      case "get":
        ...
        result = await arting.create(text,
    init_audio,
    language,
    emotion,
    speed);
