...
class Talkai {
  ...
  async chat(id = 'fb2e159b-da56-431c-b856-3a691a4a9d3c', message = 'Halo', model = 'gpt-4o-mini', temperature = 0.7) {
    const payload = {
      type: 'chat',
      messagesHistory: [
        {
...
export default async function handler(req, res) {
  ...
  const { type = 'chat', message = 'Halo', model = 'gpt-4o-mini', temperature = 0.7, size = '256x256', id } = req.query;
  ...
  try {
    ...
    if (type === 'chat') {
      ...
      return res.status(400).json({ error: 'Invalid type, must be "chat" or "image"' });
    }
