...
export default async function handler(req, res) {
  ...
  const { action, question, img } = req.query;

  try {
    switch (action) {
      case "chat":
        if (!question) {
          return res.status(400).json({ error: "Question parameter is required for chat" });
        }
      ...
      case "image":
        if (!img) {
          return res.status(400).json({ error: "Image parameter is required for image processing" });
        }
        const imageResult = await SaraStudyAI.image(img);
        return res.status(200).json({ result: imageResult });
