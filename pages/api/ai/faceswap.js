...
export default async function handler(req, res) {
  if (req.method === "GET") {
    const { sourceUrl, faceUrl } = req.query;

    if (!sourceUrl || !faceUrl) {
      return res.status(400).json({ error: "Missing sourceUrl or faceUrl" });
    }
    ...
    try {
      ...
      res.status(200).json({ resultImage: resultImageUrl });
    } catch (error) {
      console.error("Error during face swap process:", error);
      res.status(500).json({ error: "An error occurred during the face swap process." });
    }
  } else {
