...
class AiGreemAutomator {
  ...
  async getBaseUri(uploadImage) {
    if (typeof uploadImage === "string") {
      ...
      return mimeType + uploadImage.toString("base64");
    }
    return null;
  }
  ...
  async uploadImage(base64Image) {
    console.log("[🌀 UPLOAD] Mengunggah gambar ke Supa.codes...");
    try {
      ...
      if (!uploadResponse.data || !uploadResponse.data.link) {
        throw new Error("Respons upload tidak valid atau tidak ada tautan.");
      }
      console.log(`[✅ BERHASIL] Gambar diunggah ke: ${uploadResponse.data.link}`);
      return uploadResponse.data.link;
    } catch (error) {
      this._handleError("Mengunggah gambar", error);
    }
  }
