...
class StudyXAI {
  ...
  async getQuestion(
    promptInput = 'Hai',
    questionId = '10000498371',
    sessionId = '401341669243156860',
    userId = '360758167036788740',
    modelType = 14,
    event = 'pushQuestion',
    eventId = 's1734339358283',
    eventType = 2,
    paramsS2 = '10000498371',
    paramsS3 = '1',
    paramsS4 = '',
    paramsType = 14,
    askType = '',
    eventSourceType = 'web_account_homework',
    eventSourceDetail = 'https://studyx.ai/webapp/homework/10000498371'
  ) {
    ...
    switch (action) {
      case 'getShortId':
        if (!params.paramsText || !params.questionContent) {
          return res.status(400).json({ error: '`paramsText` and `questionContent` are required.' });
        }
      ...
      case 'getQuestion':
        if (!params.promptInput || !params.questionId || !params.sessionId || !params.userId) {
          return res.status(400).json({ error: '`promptInput`, `questionId`, `sessionId`, and `userId` are required.' });
        }
