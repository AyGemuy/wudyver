...
export default async function handler(req, res) {
  ...
  const {
    type = 'gptSearch', // Default type is 'gptSearch'
    chatText = 'Yoo',
    command = '',
    userName = 'User',
    email = '',
  ...
  if (!['gptSearch', 'dalle'].includes(type)) {
    return res.status(400).json({ error: 'Invalid type. Use "gptSearch" or "dalle".' });
  }
  ...
  const defaultData = {
    gptSearch: { chatText, command, userName, email },
    dalle: { prompt },
  };
  ...
  try {
    ...
    res.status(500).json({ error: `Failed to fetch data for ${type}` });
  }
}
