    ...
    try {
      ...
      main.each((i, element) => {
        ...
        const post = {
          ...
          desc:
            $(element).find("div.entry-content > p").text().trim() +
            "... Baca Selengkapnya",
          category: _.filter($(element).attr("class").split(" "), (v) =>
            v.includes("category"),
          ).map((v) => v.replace("category-", "")),
          tag: _.filter($(element).attr("class").split(" "), (v) =>
            v.includes("tag"),
          ).map((v) => v.replace("tag-", "")),
  ...
  detail = (url) => {
    return new Promise(async (resolve, reject) => {
      ...
      if (/download/.test(url)) {
        ...
        result.title = $('.pagetitle a[itemprop="item"] span[itemprop="name"]')
          .text()
          .trim();
        result.episode = $(
          '.pagetitle a[itemprop="item"] span[itemprop="name"]',
        )
          .eq(2)
          .text()
          .trim();
...
class Komiku {
  constructor() {
    (this.baseURL = "https://komiku.id"),
      (this.dataBaseURL = "https://data.komiku.id");
  }
  latest = () => {
    return new Promise(async (resolve, reject) => {
      const { data } = await axios.get(this.baseURL).catch((e) => e?.response);
      const $ = cheerio.load(data);
      const result = $("#Terbaru > div.ls4w > .ls4")
        .map((_, e) => {
          return {
            title: $(e).find("div.ls4j > h4 a").text(),
            chapter: $("div.ls4j > .ls24")
              .map((_, e) => $(e).text().trim())
              .get(),
  ...
  search = (query) => {
    return new Promise(async (resolve, reject) => {
      const { data } = await axios
        .get(this.dataBaseURL + "/cari/?post_type=manga&s=" + query)
        .catch((e) => e?.response);
      const $ = cheerio.load(data);
      const result = $("div.daftar > div.bge")
        .map((_, el) => {
          return {
...
class Komikcast {
  latest = () => {
    return new Promise(async (resolve, reject) => {
      const response = await axios.get(
        "https://komikcast.lol/daftar-komik/?orderby=update",
      );
      const $ = cheerio.load(response.data);
      const result = [];
      $(".list-update_item").each((index, element) => {
        ...
        const image = $(element)
          .find(".list-update_item-image img")
          .attr("src");
        result.push({
          title: title,
          type: type,
  ...
  detail = (url) => {
    return new Promise(async (resolve, reject) => {
      ...
      result.lastUpdated = $(".komik_info-content-update time").attr(
        "datetime",
      );
...
export default async function handler(req, res) {
  const { service, action, query, page, url} = req.query;
  ...
  try {
    ...
    switch (service) {
      case "neonime":
        if (action === "search") {
          results = await selectedService.search(query, page || 1);
        } else if (action === "ongoing") {
          results = await selectedService.ongoing(page || 1);
        } else if (action === "getData") {
          results = await selectedService.getData(url);
        }
        break;

      case "anoboy":
        if (action === "latest") {
          results = await selectedService.latest();
        } else if (action === "search") {
          results = await selectedService.search(query);
        } else if (action === "detail") {
          results = await selectedService.detail(url);
        }
        break;
      ...
      case "otakudesu":
        if (action === "latest") {
          ...
          results = await selectedService.getBatch(url);
        } else if (action === "getEpsode") {
          results = await selectedService.getEpsode(url);
        } else if (action === "getVideoFromEps") {
          results = await selectedService.getVideoFromEps(url);
        }
        break;
      ...
      case "meganei":
        if (action === "search") {
          ...
          results = await selectedService.chapter?.(url);
        } else if (action === "get") {
          results = await selectedService.get?.(url);
        } else if (action === "info") {
          results = await selectedService.info?.(url);
        } else if (action === "getChapter") {
          results = await selectedService.getChapter?.(url);
        }
        break;
