...
class Pinterest {
    ...
    async search(query, limit = 10) {
        ...
        try {
            ...
            const { data } = await axios.get(`${this.api.base}${this.api.endpoints.search}`, {
    ...
    async download(pinUrl) {
        ...
        try {
            ...
            const { data } = await axios.get(`${this.api.base}${this.api.endpoints.pin}`, {
                headers: { ...this.headers, 'cookie': cookies },
                params: params
            });
    ...
    async profile(username) {
        ...
        try {
            ...
            const { data } = await axios.get(`${this.api.base}${this.api.endpoints.user}`, {
                headers: { ...this.headers, 'cookie': cookies },
                params: params
            });
...
export default async function handler(req, res) {
  const { action, query, url, limit } = req.method === "GET" ? req.query : req.body;
  ...
  try {
    ...
    switch (action) {
      case 'search':
        ...
        result = await pinterest.search(query, searchLimit);
        break;

      case 'profile':
        if (!query) {
          return res.status(400).json({
            error: 'Parameter "query" diperlukan untuk action "profile"'
          });
        }
        result = await pinterest.profile(query);
        break;
