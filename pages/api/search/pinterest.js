...
async function pindl(url) {
  try {
    const { data } = await axios.get("https://www.savepin.app/download.php", {
      params: { url, lang: "en", type: "redirect" },
    });
...
export default async function handler(req, res) {
  try {
    if (req.method === "POST") {
      ...
      return res.status(400).json({ error: "Invalid GET parameters. Use 'url' or 'query'." });
    } else {
