...
class MLBBAPI {
  ...
  async fetchHeroes(offset = 0, limit = 10) {
    try {
      const response = await axios.get(`${this.baseURL}/heroes`, { params: { offset, limit } });
...
export default async function handler(req, res) {
  ...
  const { offset = 0, limit = 10, name, id, role } = query;
  ...
  try {
    const result = name
      ? await mlbbAPI.fetchHeroByName(name)
      : id
      ? await mlbbAPI.fetchHeroById(id)
      : role
      ? await mlbbAPI.fetchHeroesByRole(role)
      : await mlbbAPI.fetchHeroes(offset, limit);
