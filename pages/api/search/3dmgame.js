...
export default async function handler(req, res) {
  ...
  if (action === 'detail' && id) {
    try {
      ...
      return res.status(500).json({ error: 'Failed to fetch mod data' });
    }
  }

  return res.status(400).json({ error: 'Invalid action or missing query/id' });
}
...
async function getModData(id) {
  ...
  const body = JSON.stringify({ mod_id: id });
