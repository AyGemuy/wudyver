...
class EmojiService {
  async getEmojiData({ provider, query, detail, url }) {
    switch (provider) {
      case '1':
            ...
            vendors: elm
              ...
              .map((vendorElement) => {
                ...
                return {
                  name: vendorName,
                  link: vendorLink
                    ? "https://emojigraph.org" + vendorLink
                    : null,
                  image: vendorImage
                    ? "https://emojigraph.org" + vendorImage
                    : null,
                };
              }),
          };
  ...
  async emojiPedia(emoji) {
    try {
      const getSlug = await fetch(
        `https://emojipedia.org/${encodeURIComponent(emoji)}`,
        { redirect: "follow" }
      );
      const outSlug = new URL(getSlug.url).pathname.startsWith("/")
        ? new URL(getSlug.url).pathname.substring(1)
        : new URL(getSlug.url).pathname;
  ...
  async emojiGG(query) {
    ...
    try {
      ...
      const data = await response.json();
      return data.filter((s) => s.title === q || s.title?.includes(q))?.length
        ? data.filter((s) => s.title === q || s.title?.includes(q))
        : null;
    } catch (error) {
  ...
  async emojiAll(query) {
    try {
      ...
      const $ = cheerio.load(html);
      return {
        emoji: $(".emoji_card_list.pages")
          .eq(0)
          .find(".emoji_font.line")
          .first()
          .text()
          .trim() || null,
        description: $(".emoji_card_list.pages")
          .eq(0)
          .find(".emoji_card_content")
          .first()
          .text()
          .trim() || null,
        vendors: $(".emoji_card_list.pages")
          .eq(3)
          .find("ul.row.no-gutters li")
          .toArray()
          .map((el) => {
            const $el = $(el);
            ...
            return {
              ...
              image: imgSrc
                ? "https://emojiall.com" + imgSrc.replace(/\/60\//, "/240/")
                : null,
            };
          })
...
export default async function handler(req, res) {
  ...
  try {
    const { provider, query, detail, url } = req.query;
    if (!provider) {
