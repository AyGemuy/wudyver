...
const mcpedlList = async (page = 1) => {
    try {
        const { data } = await axios.get(`https://mcpedl.org/downloading/page/${page}`);
...
const mcpedlDownload = async (id) => {
    try {
        ...
        return { url: actualDownloadLink, version, size };
    } catch (error) {
...
export default async function handler(req, res) {
    const { action, page, id } = req.query;
    ...
    try {
        ...
        switch (action) {
            ...
            case "download": {
                if (!id) {
                    return res.status(400).json({ error: "ID parameter is required for download" });
                }
