...
class Search {
  tiktok = (query) => {
    return new Promise(async (resolve, reject) => {
      try {
        ...
        const videoUrls = response.data.data.videos.map(
          (video) => `https://tikwm.com${video.play}`,
        );
        resolve(videoUrls);
      } catch (error) {
        reject(error);
  ...
  xPanas = (search = "indonesia") => {
    return new Promise(async (resolve, reject) => {
      try {
        ...
        const ajg = [];
        $(
          "#content > .mozaique.thumbs-5 > center > .thumb-block > .thumb-inside > .thumb > a",
        ).each((i, u) => {
          ajg.push({
    ...
    return new Promise(async (resolve, reject) => {
      try {
        const { data } = await axios.get(
          `https://commons.wikimedia.org/w/index.php?search=${search}&title=Special:MediaSearch&go=Go&type=image`,
        );
        const $ = cheerio.load(data);
        const hasil = [];
  ...
  ringTone = (search) => {
    return new Promise(async (resolve, reject) => {
      try {
        const { data } = await axios.get(
          "https://meloboom.com/en/search/" + search,
        );
  ...
  acaraNow = () => {
    return new Promise(async (resolve, reject) => {
      try {
        const { data } = await axios.get(
          "https://www.jadwaltv.net/channel/acara-tv-nasional-saat-ini",
        );
        const $ = cheerio.load(data);
        let tv = [];
  ...
  JadwalTV = (query) => {
    return new Promise(async (resolve, reject) => {
      try {
        const { data } = await axios.get(
          "https://www.jadwaltv.net/channel/" + query,
        );
        const $ = cheerio.load(data);
        const tv = [];
        $("table.table.table-bordered > tbody > tr.jklIv").each((u, i) => {
  ...
  Steam_Detail = (url) => {
    return new Promise(async (resolve, reject) => {
      try {
        ...
        $(
          "div.game_area_sys_req.sysreq_content.active > div > ul > ul > li",
        ).each((u, i) => {
          xorizn.push($(i).text());
        });
        const hasil = $("#genresAndManufacturer")
          .html()
          .replace(/\n/g, "")
          .replace(/<br>/g, "\n")
          .replace(/\t/g, "")
          ...
          .replace(/<\/b>/g, " ")
          .replace(/<[^>]*>/g, "");
        const desc = $("div.game_description_snippet")
          .text()
          .replace(/\t/g, "")
          .replace(/\n/g, "");
        const hasill = {
    ...
    return new Promise(async (resolve, reject) => {
      try {
        const { data } = await axios.get(
          ...
          _data = [];
        $(
          ".story-card-container > ul.list-group.new-list-group > li.list-group-item",
        ).each(function (i, u) {
          let link = limk + $(u).find("a").attr("href");
          let judul = $(u)
            .find("a > div > div.story-info > div.title")
            .text()
            .trim();
          let img = $(u).find("a > div > div.cover > img").attr("src");
          let desc = $(u)
            .find("a > div > div.story-info > .description")
            .text()
            .replace(/\s+/g, " ");
          let _doto = [];
          $(u)
            .find("a > div > div.story-info > .new-story-stats > .stats-item")
            .each((u, i) => {
              _doto.push(
                $(i).find(".icon-container > .tool-tip > .sr-only").text(),
              );
            });
          _data.push({
            title: judul,
  ...
  kbbi = (query) => {
    return new Promise(async (resolve, reject) => {
      try {
        const { data } = await axios.get(
          "https://kbbi.kemdikbud.go.id/entri/" + query,
        );
        const $ = cheerio.load(data);
        let _kata = [];
        let _arti = [];
  ...
  Nomina = (query) => {
    return new Promise(async (resolve, reject) => {
      try {
        const { data } = await axios.get(
          "https://tesaurus.kemdikbud.go.id/tematis/lema/" + query + "/nomina",
        );
        const $ = cheerio.load(data);
        let _arti = [];
        $(".search-result-area > .result-par > .contain > .result-set").each(
          (i, u) => {
  ...
  KodePos = (query) => {
    return new Promise(async (resolve, reject) => {
      try {
        const { data } = await axios.get(
          "https://nomorkodepos.com/?s=" + query,
        );
        const $ = cheerio.load(data);
        let _data = [];
        $("table.pure-table.pure-table-horizontal > tbody > tr").each(
          (i, u) => {
            let _doto = [];
            $(u)
              .find("td")
    ...
    return new Promise(async (resolve, reject) => {
      try {
        ...
        $("table.wikitable.sortable > tbody > tr").each((i, u) => {
          let hero_icon = $(u)
            .find("td:nth-child(1) > center > a > img")
            .attr("data-src");
    ...
    return new Promise(async (resolve, reject) => {
      try {
        ...
        if (status === 200) {
          ...
          _$(".pi-item.pi-data.pi-item-spacing.pi-border-color").each(
            (l, m) => {
              rell.push(
                _$(m).text().trim().replace(/\n/g, ":").replace(/\t/g, ""),
              );
            },
          );
        ...
        try {
          ...
          $(".item-container a.current-item-photo img").each((i, element) => {
            ...
            if (srcset) {
              ...
              const hdUrl = _.find(urls, (url) => url.includes("2x")).split(
                " ",
              )[0];
              if (hdUrl) {
...
export default async function handler(req, res) {
  ...
  try {
    ...
    switch (action) {
      case "tiktok":
        results = await search.tiktok(query);
        break;
      case "sfile":
        results = await search.sfile(query);
        break;
      case "apkmirror":
        results = await search.apkmirror(query);
        break;
      case "musixmatch":
        results = await search.musixmatch(query);
        break;
      case "chord":
        results = await search.chord(query);
        break;
      case "xPanas":
        results = await search.xPanas(query || "indonesia");
        break;
      case "wikiMedia":
        results = await search.wikiMedia(query);
        break;
      case "soundCloude":
        results = await search.soundCloude(query);
        break;
      case "ringTone":
        results = await search.ringTone(query);
        break;
      case "playStore":
        results = await search.playStore(query);
        break;
      case "tixID":
        results = await search.tixID();
        break;
      case "bukaLapak":
        results = await search.bukaLapak(query);
        break;
      case "acaraNow":
        results = await search.acaraNow();
        break;
      case "jadwal_Sepakbola":
        results = await search.jadwal_Sepakbola();
        break;
      case "JadwalTV":
        results = await search.JadwalTV(query);
        break;
      case "Steam":
        results = await search.Steam(query);
        break;
      case "Steam_Detail":
        results = await search.Steam_Detail(url);
        break;
      case "WattPad":
        results = await search.WattPad(query);
        break;
      case "linkWa":
        results = await search.linkWa(query);
        break;
      case "kbbi":
        results = await search.kbbi(query);
        break;
      case "Nomina":
        results = await search.Nomina(query);
        break;
      case "KodePos":
        results = await search.KodePos(query);
        break;
      case "listHero":
        results = await search.listHero();
        break;
      case "hero":
        results = await search.hero(query);
        break;
      case "wallpaperHd":
        results = await search.wallpaperHd(query, page || 5);
        break;
      default:
