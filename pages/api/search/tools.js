...
class Tools {
    ...
    return new Promise(async (resolve, reject) => {
      axios("https://www.editpad.org/tool/tool/summarizingTool", {
        ...
        data:
          "text=" +
          text +
          "&percnt=50&modd=1&min_length=84.75&max_length=135.6&captcha=0",
        method: "POST",
      }).then((yanz) => {
        resolve(yanz.data);
      });
  ...
  code = (text, input, code) => {
    return new Promise(async (resolve, reject) => {
      try {
        const { data } = axios("https://codepal.ai/api/code-generator", {
          headers: {
            "Content-Type": "application/json",
            "User-Agent":
              "Mozilla/5.0 (Linux; Android 10; K) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/116.0.0.0 Mobile Safari/537.36",
  ...
  webpage = (text) => {
    return new Promise(async (resolve, reject) => {
      try {
        const response = await axios.post(
          "https://codepal.ai/api/live-webpage-generator",
          {
            code: text,
...
export default async function handler(req, res) {
  const { action, query, url, device, type, input, code, model, voice } = req.query;
  ...
  try {
    ...
    switch (action) {
      case "ssweb":
        results = await tools.ssweb(url, device || "desktop");
        break;
      case "speech":
        results = await tools.speech(query, type);
        break;
      case "webpage":
        results = await tools.webpage(query);
        break;
      case "code":
        results = await tools.code(query, input, code);
        break;
      case "blacbox":
        results = await tools.blacbox(query);
        break;
      case "SimSimi":
        results = await tools.SimSimi(query);
        break;
      case "rangkum":
        results = await tools.rangkum(query);
        break;
      case "detectText":
        results = await tools.detectText(query);
        break;
      case "elevenlabs":
        results = await tools.elevenlabs(query, model);
        break;
      case "detectLang":
        results = await tools.detectLang(query);
        break;
      case "elevenModel":
        results = await tools.elevenModel(query);
        break;
      case "soundText":
        results = await tools.soundText(query, voice);
        break;
      default:
