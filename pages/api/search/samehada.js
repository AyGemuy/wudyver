...
class Samehadaku {
  ...
  async search(query) {
    try {
      const res = await fetch(`${this.BASE_URL}/?${new URLSearchParams({ s: query })}`);
  ...
  async release() {
    try {
      ...
      for (let day of Object.keys(data)) {
        const res = await fetch(`${this.BASE_URL}/wp-json/custom/v1/all-schedule?${new URLSearchParams({ perpage: "20", day: day, type: "schtml" })}`);
...
export default async function handler(req, res) {
  ...
  if (method === 'GET') {
    ...
    try {
      if (action === 'search') {
        const results = await samehadaku.search(query);
        return res.status(200).json(results);
      } else if (action === 'latest') {
        const results = await samehadaku.latest();
        return res.status(200).json(results);
      } else if (action === 'release') {
        const results = await samehadaku.release();
        return res.status(200).json(results);
      } else if (action === 'detail') {
        const results = await samehadaku.detail(query); // Assuming query is the URL
        return res.status(200).json(results);
      } else if (action === 'download') {
        const results = await samehadaku.download(query); // Assuming query is the URL
        return res.status(200).json(results);
      } else {
