...
export default async function handler(req, res) {
  ...
  if (action === "search") {
    if (!q) {
      return res.status(400).json({ error: "Parameter 'q' diperlukan untuk pencarian." });
    }
    try {
      const response = await fetch(
        `${randomProxyUrl}https://www.youporn.com?query=${q}`
      );
      const body = await response.text();
      const $ = cheerio.load(body);

      const results = $(".video-box.pc.js_video-box")
        .map((_, el) => {
          const $el = $(el);
          return {
      ...
      return res.status(500).json({ error: "Terjadi kesalahan saat pencarian video." });
    }
  }

  if (action === "download") {
    if (!id) {
      return res.status(400).json({ error: "Parameter 'id' diperlukan untuk download." });
    }
    try {
      const response = await fetch(`https://www.youporn.com/watch/${id}`);
      const text = await response.text();

      const jsonDataMatch = text
        .split("page_params.video_player_setup")[1]
        ?.split("page_params.video.playerParams")[0]
        ?.match(/(\{(?:[^{}"\\]|\\.)*\}|(?:\[.*?\]))/g);
      ...
      if (jsonDataMatch) {
        ...
        return res.status(500).json({ error: "Gagal menemukan data JSON video." });
      }
    } catch (error) {
      console.error("Error fetching video data:", error);
      return res.status(500).json({ error: "Terjadi kesalahan saat mengunduh video." });
    }
  }
