...
class AkinatorHandler {
  ...
  async startGame() {
    ...
    const answers = [
      response.data.match(/id="a_yes".*?>(.+)<\/a>/)?.[1]?.trim(),
      response.data.match(/id="a_no".*?>(.+)<\/a>/)?.[1]?.trim(),
      response.data.match(/id="a_dont_know".*?>(.+)<\/a>/)?.[1]?.trim(),
      response.data.match(/id="a_probably".*?>(.+)<\/a>/)?.[1]?.trim(),
      response.data.match(/id="a_probaly_not".*?>(.+)<\/a>/)?.[1]?.trim()
    ];

    return { question, session, signature, answers };
  }
...
export default async function handler(req, res) {
  ...
  try {
    switch (action) {
      case 'start': {
        const { question, session, signature, answers } = await akiHandler.startGame();
      ...
      case 'step': {
        if (!sessionId || !answer) {
          return res.status(400).json({ success: false, error: 'Parameter id dan answer diperlukan.' });
        }
      ...
      case 'delete': {
        ...
        return res.status(200).json({ success: true, message: 'Session berhasil dihapus.' });
      }
