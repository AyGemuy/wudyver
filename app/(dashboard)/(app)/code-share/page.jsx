...
const CodeSharePage = () => {
  ...
  const [showAddCodeModal, setShowAddCodeModal] = useState(false); // State untuk modal
  ...
  const handleAddCode = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setShowAddCodeModal(false); // Tutup modal setelah berhasil
        fetchCodes();
      } else {
  ...
  const updateCodeInteraction = async (id, actionType) => {
    try {
      ...
      if (response.ok) {
        ...
        // Notifikasi untuk like/dislike bisa di-skip jika terlalu berisik,
        // atau dibuat lebih subtle. Untuk view, biasanya tidak ada notifikasi.
        // if (actionType === 'like') toast.success("Kode disukai!");
        // else if (actionType === 'dislike') toast.info("Kode tidak disukai.");
      } else {
        const error = await response.json();
  ...
  const handleClearAll = async () => {
    if (!confirm("Apakah Anda yakin ingin menghapus SEMUA kode? Tindakan ini tidak dapat diurungkan.")) return;
    setLoading(true);
    try {
      const response = await fetch('/api/code-share', {
        method: 'DELETE',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ clearAll: true }), // Kirim flag clearAll
      });
      if (response.ok) {
    ...
    <>
      ...
      <div className="w-full px-2 sm:px-4 py-4">
        <Card
          ...
          className="w-full border border-emerald-500/50 dark:border-emerald-600/70 rounded-xl shadow-xl bg-white text-slate-800 dark:bg-slate-800/50 dark:text-slate-100 backdrop-blur-sm bg-opacity-80 dark:bg-opacity-80"
          ...
          <div className="p-3 sm:p-4 border-b border-slate-200 dark:border-slate-700/60 shrink-0">
            <div className="flex flex-col sm:flex-row items-center justify-between gap-2">
                <div className="flex items-center">
                    <div className="w-9 h-9 sm:w-10 sm:h-10 flex items-center justify-center rounded-full bg-gradient-to-br from-teal-500 to-emerald-600 text-white shadow-md mr-2.5 sm:mr-3 shrink-0">
                        <Icon icon="ph:code-block-duotone" className="text-xl sm:text-2xl" />
                    </div>
                    <div>
                        <h4 className="text-base sm:text-lg md:text-xl font-bold bg-clip-text text-transparent bg-gradient-to-r from-teal-500 to-emerald-500">
                        Manajer Berbagi Kode
                        </h4>
                        <p className="text-xs text-slate-500 dark:text-slate-400 mt-0.5">Kelola dan bagikan cuplikan kode Anda</p>
                    </div>
                </div>
            </div>
          </div>
          ...
          <SimpleBar className="flex-grow overflow-y-auto">
            <div className="p-3 sm:p-4">
              ...
              <div className="flex flex-wrap gap-2 sm:gap-3 mb-4">
                <Button
                  onClick={() => setShowAddCodeModal(true)} // Buka modal
                    ...
                    return (
                      <div key={code._id} className="bg-white dark:bg-slate-800/60 p-3 sm:p-4 rounded-lg border border-slate-200 dark:border-slate-700/50 shadow-sm hover:shadow-md transition-shadow duration-150">
                        <div className="flex justify-between items-start mb-2 sm:mb-3">
                          <div className="flex-grow min-w-0">
                            ...
                            <div className="flex flex-wrap items-center gap-x-2 gap-y-1 mt-1 text-[10px] sm:text-xs text-slate-500 dark:text-slate-400">
                              ...
                              <span className="flex items-center cursor-pointer hover:text-emerald-500 dark:hover:text-emerald-300 transition-colors" onClick={() => handleView(code._id)} title="Klik untuk melihat dan menambah jumlah dilihat">
                        ...
                        <div className="bg-slate-100 dark:bg-slate-900/70 rounded-md p-2.5 sm:p-3 border border-slate-200 dark:border-slate-700/80 relative mt-1.5">
                          <pre className="text-slate-700 dark:text-slate-200 text-[11px] sm:text-xs font-mono whitespace-pre-wrap break-words overflow-x-auto max-h-52 sm:max-h-72 simple-scrollbar">
      ...
      <Modal
        ...
        onClose={() => {
            ...
            setNewCode({ author: "", fileName: "", code: "", tag: "" }); // Reset form saat modal ditutup
        }}
        themeClass="bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700/60 rounded-lg shadow-xl"
        headerClass="bg-gradient-to-r from-teal-500 to-emerald-600 text-white"
        ...
        footerContent={
          <div className="flex justify-end space-x-2">
            ...
            />
            <Button
              onClick={handleAddCode} // Tidak perlu type="submit" karena onClick sudah ada
              text="Simpan Kode"
              className={`${buttonGradientBase} bg-gradient-to-r from-emerald-500 to-teal-500 hover:from-emerald-600 hover:to-teal-600`}
      ...
      >
        <div className="space-y-3 text-slate-800 dark:text-slate-100 p-0.5">
            <Textinput 
                ...
                description="Nama pembuat cuplikan kode."
            />
            <Textinput 
                label="Nama Berkas *"
                ...
                className={inputBaseClass}
                labelClass={labelBaseClass}
                description="Nama berkas untuk identifikasi."
            />
            <Textinput 
                label="Tag (Opsional)"
                ...
                className={inputBaseClass}
                labelClass={labelBaseClass}
                description="Kategori atau bahasa kode."
            />
            <div>
                ...
                <textarea
                    ...
                    rows={8}
                    className={`${inputBaseClass} font-mono text-xs leading-relaxed min-h-[120px]`}
                />
                <p className="text-[10px] text-slate-500 dark:text-slate-400 mt-0.5">Isi dengan cuplikan kode yang ingin Anda bagikan.</p>
            </div>
        </div>
      </Modal>
    </>
