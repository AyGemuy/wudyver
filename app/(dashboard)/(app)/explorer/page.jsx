...
const APIExplorerPage = () => {
    ...
    const fetchSpec = async () => {
      ...
      try {
        ...
        if (!res.ok) {
          ...
          throw new Error(`Gagal mengambil spesifikasi: ${res.status} ${res.statusText}. ${errorText.substring(0, 100)}`);
        }
        ...
        Object.entries(paths).forEach(([path, methods]) => {
          Object.entries(methods).forEach(([method, details]) => {
            ...
            if (!grouped[endpointTag]) {
              grouped[endpointTag] = {
                description: tagsFromSpec.find(t => t.name === endpointTag)?.description || "Endpoint dalam grup ini.",
                endpoints: []
              };
            }
            grouped[endpointTag].endpoints.push({ path, method: method.toUpperCase(), details });
          });
        });
  ...
  const executeTryIt = async (formData) => {
    ...
    if (["POST", "PUT", "PATCH"].includes(methodToExecute) && formData.requestBodyParams?.length > 0) {
        ...
        let rawBodyValue = "";
        if (formData.requestBodyParams.length === 1 && formData.requestBodyParams[0].key === "") {
            rawBodyValue = formData.requestBodyParams[0].value;
        }

        if (effectiveContentType === "application/json") {
            if (rawBodyValue) {
                try { requestBodyData = JSON.parse(rawBodyValue); } catch (e) { toast.error(`JSON tidak valid pada request body.`); setTryItError("JSON tidak valid pada request body."); setTryItLoading(false); return; }
            } else {
                formData.requestBodyParams.forEach(p => {
                    if (p.key) {
                        try {
                            if (p.value.trim().startsWith('{') || p.value.trim().startsWith('[')) requestBodyData[p.key] = JSON.parse(p.value);
                            else if (p.value === "true") requestBodyData[p.key] = true;
                            else if (p.value === "false") requestBodyData[p.key] = false;
                            else if (!isNaN(p.value) && p.value.trim() !== "" && !p.value.startsWith("0") && p.value.length < 16) requestBodyData[p.key] = Number(p.value);
    ...
    try {
      ...
      if (!res.ok) {
        ...
        if (contentTypeHeader?.includes("application/json")) { setTryItResponse(await res.json()); setTryItResponseType("json"); }
        ...
        else if (contentTypeHeader?.startsWith("image/")) { const b = await res.blob(); const newUrl = URL.createObjectURL(b); setTryItResponseUrl(newUrl); setTryItResponse(newUrl); setTryItResponseType("image"); }
        else if (contentTypeHeader?.startsWith("video/")) { const b = await res.blob(); const newUrl = URL.createObjectURL(b); setTryItResponseUrl(newUrl); setTryItResponse(newUrl); setTryItResponseType("video"); }
          ...
          const b = await res.blob(); const newUrl = URL.createObjectURL(b); setTryItResponseUrl(newUrl);
          let fn = "downloaded_file"; const cd = res.headers.get("content-disposition"); if (cd) { const m = cd.match(/filename="?([^"]+)"?/); if (m && m[1]) fn = m[1];}
          setTryItResponse({ url: newUrl, filename: fn }); setTryItResponseType("document");
        } else {
          try {
            const b = await res.blob(); const newUrl = URL.createObjectURL(b); setTryItResponseUrl(newUrl);
            let fn = "downloaded_blob"; const cd = res.headers.get("content-disposition"); if (cd) { const m = cd.match(/filename="?([^"]+)"?/); if (m && m[1]) fn = m[1];}
            setTryItResponse({ url: newUrl, filename: fn }); setTryItResponseType("blob");
          } catch { setTryItResponse(await res.text()); setTryItResponseType("text"); }
        }
    ...
    <div className="space-y-5 text-slate-800 dark:text-slate-100">
      <div>
        ...
        <select {...register("method")} className={inputBaseClass}>
          {Object.keys(badgeMethodColors).map(m => (<option key={m} value={m}>{m}</option>))}
        </select>
      </div>
        ...
        <div className="space-y-3">
          {queryFields.map((f, i) => (
            <div key={f.id} className="flex flex-col sm:flex-row items-start sm:items-center gap-2">
              <input type="text" {...register(`queryParams.${i}.name`, { required: f.required && watch(`queryParams.${i}.value`) !== "" })} placeholder="Nama Param" className={`${inputBaseClass} sm:flex-1`} />
      ...
      <div>
        ...
        <div className="space-y-3">
          {headerFields.map((f, i) => (
            <div key={f.id} className="flex flex-col sm:flex-row items-start sm:items-center gap-2">
              <input type="text" {...register(`headerParams.${i}.name`, { required: f.required && watch(`headerParams.${i}.value`) !== "" })} placeholder="Nama Header" className={`${inputBaseClass} sm:flex-1`} />
  ...
  const copyResponseToClipboard = async () => {
    ...
    if (tryItResponseType === "json") { textToCopy = JSON.stringify(tryItResponse, null, 2); toastMessage = "Response JSON disalin!"; }
    else if (tryItResponseType === "text") { textToCopy = String(tryItResponse); toastMessage = "Response teks disalin!"; }
    else if ((tryItResponseType === "image" || tryItResponseType === "video" || tryItResponseType === "document" || tryItResponseType === "blob") && (tryItResponse?.url || typeof tryItResponse === 'string')) {
      textToCopy = tryItResponse?.url || tryItResponse; toastMessage = "URL Response disalin!";
    }
    if (textToCopy) {
  ...
  const currentSyntaxTheme = typeof window !== 'undefined' && window.matchMedia && window.matchMedia('(prefers-color-scheme: dark)').matches ? atomOneDark : atomOneLight;
    ...
    <div className="w-full px-2 sm:px-4 md:px-6 py-6">
      ...
      >
        <div className="p-4 sm:p-6 border-b border-slate-200 dark:border-slate-700/60">
          <div className="flex flex-col sm:flex-row items-center">
            <div className="w-10 h-10 sm:w-11 sm:h-11 flex items-center justify-center rounded-full bg-gradient-to-br from-teal-500 to-cyan-600 text-white shadow-md mr-0 sm:mr-3 mb-2 sm:mb-0 shrink-0">
        ...
        <SimpleBar className="h-full" style={{ maxHeight: 'calc(100vh - 160px)' }}>
          <div className="p-4 sm:p-6">
            {loading ? (
              <div className="flex flex-col items-center justify-center p-10 min-h-[calc(100vh-300px)]">
                <Icon icon="svg-spinners:blocks-shuffle-3" className="text-5xl sm:text-6xl text-teal-500 mb-4" />
                <p className="text-lg sm:text-xl font-medium text-slate-600 dark:text-slate-300">Memuat Spesifikasi API...</p>
              </div>
            ) : apiFetchError ? (
              <div className="flex flex-col items-center justify-center p-6 sm:p-10 min-h-[calc(100vh-300px)] bg-red-50 dark:bg-red-800/20 rounded-lg">
                ...
                <p className="text-lg sm:text-xl font-semibold text-red-700 dark:text-red-300">Gagal Memuat Data API Explorer</p>
                <p className="text-sm text-red-600 dark:text-red-400 mt-2 text-center max-w-xl">{apiFetchError}</p>
              </div>
            ) : sortedTagKeys.length === 0 ? (
              <div className="flex flex-col items-center justify-center p-10 min-h-[calc(100vh-300px)] text-slate-500 dark:text-slate-400">
                <Icon icon="ph:binoculars-duotone" className="text-6xl mb-4 opacity-70" />
                <p className="text-lg sm:text-xl">Tidak Ada Endpoint API yang Ditemukan.</p>
                <p className="text-sm mt-1">Spesifikasi API mungkin kosong atau tidak tersedia.</p>
              </div>
            ) : (
              <div className="space-y-3 sm:space-y-4">
                {sortedTagKeys.map((tagKey) => (
                  <Disclosure key={tagKey} as="div" className="bg-slate-100 dark:bg-slate-800/60 rounded-xl border border-slate-200 dark:border-slate-700/60 shadow-sm" defaultOpen={false}>
                    {({ open }) => (
                      <>
                        <Disclosure.Button className="flex justify-between items-center w-full px-4 py-3 sm:px-5 sm:py-3.5 text-left rounded-t-xl group focus:outline-none focus-visible:ring-2 focus-visible:ring-teal-500 focus-visible:ring-opacity-75 hover:bg-slate-200 dark:hover:bg-slate-700/80 transition-colors duration-150 data-[headlessui-state=open]:bg-slate-200 data-[headlessui-state=open]:dark:bg-slate-700/80 data-[headlessui-state=open]:rounded-b-none">
                          <div className="flex items-center min-w-0">
                            ...
                            <div className="flex-grow min-w-0">
                              <span className="font-semibold text-sm sm:text-base text-teal-700 dark:text-teal-200 truncate" title={tagKey}>{tagKey}</span>
                              <p className="text-xs text-slate-500 dark:text-slate-400 truncate" title={apis[tagKey]?.description}>
                                {apis[tagKey]?.description || `(${apis[tagKey].endpoints.length} endpoint)`}
                              </p>
                            </div>
                          </div>
                          <div className="flex items-center">
                            <span className="text-xs px-2 py-0.5 bg-teal-100 dark:bg-teal-700/60 text-teal-600 dark:text-teal-300 rounded-full mr-2 sm:mr-3">
                              {apis[tagKey].endpoints.length}
                            </span>
                            <Icon icon="ph:caret-down-bold" className={`${open ? "transform rotate-180" : ""} w-4 h-4 sm:w-5 sm:h-5 text-teal-500 dark:text-teal-400 transition-transform`}/>
                          </div>
                        </Disclosure.Button>
                        <Transition as={Fragment} enter="transition ease-out duration-100" enterFrom="opacity-0 -translate-y-2" enterTo="opacity-100 translate-y-0" leave="transition ease-in duration-75" leaveFrom="opacity-100 translate-y-0" leaveTo="opacity-0 -translate-y-2">
                          <Disclosure.Panel className="px-3 pt-2 pb-3 sm:px-4 sm:pt-3 sm:pb-4 text-sm bg-white dark:bg-slate-800/30 rounded-b-xl border-t border-slate-200 dark:border-slate-700/60">
                            <div className="space-y-1.5">
                            {apis[tagKey].endpoints.map((api, idx) => (
                              <Disclosure key={`${tagKey}-${idx}`} as="div" className="bg-slate-50 dark:bg-slate-700/50 rounded-md border border-slate-200 dark:border-slate-600/70 shadow-sm">
                                {({ open: openEndpoint }) => (
                                  <>
                                    <Disclosure.Button className="w-full text-left flex items-center justify-between p-2.5 sm:p-3 group focus:outline-none focus-visible:ring-1 focus-visible:ring-teal-500 focus-visible:ring-opacity-75 hover:bg-slate-100 dark:hover:bg-slate-700/70 transition-colors duration-150 data-[headlessui-state=open]:bg-slate-100 data-[headlessui-state=open]:dark:bg-slate-700/70 data-[headlessui-state=open]:rounded-b-none rounded-md">
                                      <div className="flex items-center min-w-0">
                                        <span className={`inline-block font-semibold text-[11px] sm:text-xs px-2 py-1 rounded mr-2 sm:mr-2.5 ${badgeMethodColors[api.method.toUpperCase()] || 'bg-gray-200 text-gray-700'}`}>
                                          {api.method.toUpperCase()}
                                        </span>
                                        <span className={`truncate text-xs sm:text-sm font-mono ${openEndpoint ? "text-teal-700 dark:text-teal-200 font-medium" : "text-slate-700 dark:text-slate-300"}`}>
                                          {api.path}
                                        </span>
                                      </div>
                                      <Icon icon="ph:caret-down-bold" className={`${openEndpoint ? "transform rotate-180" : ""} w-3.5 h-3.5 text-teal-500 dark:text-teal-400 ml-2 transition-transform duration-200`}/>
                                    </Disclosure.Button>
                                    <Transition as={Fragment} enter="transition ease-out duration-100" enterFrom="opacity-0 -translate-y-1" enterTo="opacity-100 translate-y-0" leave="transition ease-in duration-75" leaveFrom="opacity-100 translate-y-0" leaveTo="opacity-0 -translate-y-1">
                                      <Disclosure.Panel className="bg-white dark:bg-slate-700/30 text-xs sm:text-sm rounded-b-md p-3 sm:p-4 space-y-3 border-t border-slate-200 dark:border-slate-600/70">
                                        <div className="prose prose-sm dark:prose-invert max-w-none prose-p:my-1 prose-headings:my-1.5 prose-headings:text-xs prose-headings:font-medium prose-headings:text-slate-500 prose-headings:dark:text-slate-400">
                                          <h5>Ringkasan</h5>
                                          <p>{api.details.summary || "Tidak ada ringkasan."}</p>
                                          {api.details.description && (<><h5>Deskripsi</h5><p>{api.details.description}</p></>)}
                                        ...
                                        <div className="mt-3 flex justify-end">
                                          <Button onClick={() => openTryItModal(api)} text="Coba!" icon="ph:play-circle-duotone" iconClassName="mr-1" className="bg-teal-500 hover:bg-teal-600 text-white text-xs py-1.5 px-3 rounded-md shadow-sm transition-colors"/>
                                        </div>
                                      </Disclosure.Panel>
                                    </Transition>
                                  </>
      ...
      {currentApiEndpoint && (
        <Modal
          title={
            <div className="flex items-center text-slate-900 dark:text-slate-50">
              ...
              <div className="flex min-w-0 flex-1 flex-wrap items-baseline gap-x-1.5">
                ...
                <span className={`inline-block whitespace-nowrap font-semibold text-[10px] sm:text-xs px-1.5 py-0.5 rounded ${badgeMethodColors[currentApiEndpoint.method.toUpperCase()] || 'bg-gray-200 text-gray-700'}`}>
          ...
          className="max-w-3xl lg:max-w-4xl xl:max-w-5xl" // Disesuaikan agar modal "Try It" lebih lebar
          ...
          footerContent={
            <div className="flex flex-col sm:flex-row justify-end w-full gap-3">
              <Button type="button" text="Tutup" onClick={() => setShowTryItModal(false)} className={`${buttonSecondaryClass} w-full sm:w-auto px-4 py-2 text-sm`} />
              <Button
                type="submit"
                ...
                className="bg-gradient-to-r from-teal-500 to-cyan-600 hover:from-teal-600 hover:to-cyan-700 text-white w-full sm:w-auto px-4 py-2 text-sm shadow-sm hover:shadow-md flex items-center justify-center"
            ...
            <form onSubmit={handleSubmit(executeTryIt)} className="space-y-4 p-0.5">
              ...
              {tryItError && (
                <div className="mt-4 p-3 bg-red-100/80 border border-red-300 text-red-700 rounded-lg dark:bg-red-900/50 dark:border-red-700/60 dark:text-red-300 text-xs">
                  <h5 className="font-semibold mb-1 flex items-center text-sm"><Icon icon="ph:warning-circle-duotone" className="mr-2 text-lg" /> Error:</h5>
                  <pre className="whitespace-pre-wrap break-all font-mono">{tryItError}</pre>
                </div>
              )}
              {tryItResponse !== null && !tryItError && (
                <div className="mt-4">
                  <div className="flex justify-between items-center mb-2">
                    ...
                    {(tryItResponseType === "json" || tryItResponseType === "text" || (["image", "video", "document", "blob"].includes(tryItResponseType) && (tryItResponse?.url || typeof tryItResponse === 'string'))) && (
                      <Button
                        onClick={copyResponseToClipboard}
                        text={<><Icon icon={responseCopied ? "ph:check-circle-duotone" : "ph:copy-duotone"} className="mr-1 text-sm" /> {responseCopied ? "Disalin!" : (tryItResponseType === "json" || tryItResponseType === "text" ? "Salin" : "Salin URL")}</>}
                        className={`${responseCopied ? 'bg-green-500 hover:bg-green-600' : 'bg-slate-400 hover:bg-slate-500'} text-white py-1 px-2 rounded text-[10px] dark:bg-slate-600 dark:hover:bg-slate-500 transition-colors duration-150`}
                      />
                    )}
                  </div>
                  <div className="border border-slate-200 dark:border-slate-700/80 rounded-lg overflow-hidden">
                    {tryItResponseType === "json" && (
                        <SyntaxHighlighter language="json" style={currentSyntaxTheme} customStyle={{ margin: 0, padding: '0.75rem', borderRadius: '0px'}} className="text-xs max-h-96 overflow-auto simple-scrollbar">
                    ...
                    {tryItResponseType === "image" && typeof tryItResponse === 'string' && ( <img src={tryItResponse} alt="API Response" className="max-w-full h-auto block p-2 bg-slate-50 dark:bg-slate-800/70" /> )}
                    ...
                    {(tryItResponseType === "document" || tryItResponseType === "blob") && tryItResponse?.url && (
                      <div className="p-3 bg-slate-50 dark:bg-slate-800/70">
                        <p className="text-xs sm:text-sm text-slate-700 dark:text-slate-300 mb-2"> Menerima berkas: <span className="font-medium">{tryItResponse.filename || "download"}</span> </p>
                        <a href={tryItResponse.url} download={tryItResponse.filename || "download"} className="inline-flex items-center px-3 py-1.5 bg-teal-500 hover:bg-teal-600 text-white text-xs font-medium rounded-md transition-colors"> <Icon icon="ph:download-simple-duotone" className="mr-1.5 text-base" /> Unduh Berkas </a>
