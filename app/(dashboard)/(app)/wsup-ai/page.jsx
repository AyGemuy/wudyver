...
const WSUP_AI_API_BASE = "/api/ai/wsup";

const WsupAiChatPage = () => {
  ...
  const callWsupApi = async (url, method = "GET", body = null) => {
    ...
    try {
      ...
      console.error("Wsup AI API call error:", err);
  ...
  const handleSearchCharacter = async (e) => {
    ...
    const response = await callWsupApi(`${WSUP_AI_API_BASE}?action=search&query=${encodeURIComponent(searchTerm)}`);
    
    if (response.success && response.data && response.data.body && Array.isArray(response.data.body)) {
      const formattedResults = response.data.body.map(char => ({
        id: char.id,
        name: char.name || "Nama Tidak Tersedia",
        description: char.description || "Deskripsi tidak tersedia.",
        characterImageUrl: char.characterImageUrl || "/assets/images/users/user-0.jpg",
        chatSessionCount: char.chatSessionCount || 0,
        createdBy: char.createdBy || "Tidak diketahui",
      }));
      ...
      setSearchResults([]);
      if (response.success && !(response.data && response.data.body && Array.isArray(response.data.body))) {
        toast.error("Format data pencarian tidak sesuai.");
      }
    }
  };
  ...
  const handleSendMessage = async (e) => {
    ...
    const response = await callWsupApi(
      `${WSUP_AI_API_BASE}?action=chat&char_id=${selectedCharacter.id}&prompt=${encodeURIComponent(currentPrompt)}`,
      "GET"
    );

    if (response.success && response.data && response.data.body && typeof response.data.body.answer === 'string') {
      setChatMessages((prevMessages) => [
        ...prevMessages, {
          id: Date.now() + 1,
          sender: "ai",
          text: response.data.body.answer,
          ...
          <div className="p-4 sm:p-6 border-b border-slate-200 dark:border-slate-700/60">
            <div className="flex flex-col sm:flex-row items-center">
              <div className="w-10 h-10 sm:w-12 sm:h-12 flex items-center justify-center rounded-full bg-gradient-to-br from-teal-500 to-cyan-600 text-white shadow-md mb-2 sm:mb-0">
                <Icon icon="ph:wechat-logo-duotone" className="text-2xl sm:text-3xl" />
              </div>
            ...
            <p className="text-sm text-center sm:text-left text-slate-500 dark:text-slate-400 mt-2 ml-0 sm:ml-[calc(2.5rem+1rem)] md:ml-[calc(3rem+1rem)]">
              Ngobrol santai dengan karakter AI dari Wsup!
            </p>
          </div>
