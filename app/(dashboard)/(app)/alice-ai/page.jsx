...
const ALICE_AI_API_BASE = "/api/ai/alice-ai";

const AliceAiChatPage = () => {
  ...
  const callAliceApi = async (url, method = "GET", bodyPayload = null) => {
    ...
    try {
      ...
      if (responseData.code === 0 && responseData.data && responseData.data.items !== undefined) {
      ...
      const errorMessage = responseData.msg || responseData.message || (responseData.code !== undefined ? `Error (code: ${responseData.code})` : "Operasi gagal atau format respons tidak dikenal.");
      toast.error(errorMessage);
      return { success: false, message: errorMessage };

    } catch (err) {
      console.error("Alice AI API call error:", err);
  ...
  const handleSearchCharacter = async (e) => {
    ...
    const apiResponse = await callAliceApi(`${ALICE_AI_API_BASE}?action=search&query=${encodeURIComponent(searchTerm)}`);

    if (apiResponse.success && Array.isArray(apiResponse.data)) {
      const formattedResults = apiResponse.data.map(item => ({
        id: item.chatbot_id,
        name: item.nickname,
        description: item.short_description || 'Tidak ada deskripsi.',
        characterImageUrl: item.avatar ? (item.avatar.startsWith('http') ? item.avatar : `https://oss-prod.aliceprj.com${item.avatar}`) : "/assets/images/users/user-0.jpg",
        chatSessionCount: item.message_count || 0,
        createdBy: item.like_count || 0,
      }));
      ...
      if (apiResponse.success && apiResponse.data === null) {
        toast.info("Tidak ada karakter yang ditemukan atau format data tidak sesuai.");
  ...
  const handleSendMessage = async (e) => {
    ...
    const apiResponse = await callAliceApi(
      `${ALICE_AI_API_BASE}?action=chat&char_id=${selectedCharacter.id}&prompt=${encodeURIComponent(currentPrompt)}`,
      "GET"
    );

    if (apiResponse.success && apiResponse.data && typeof apiResponse.data.result === 'string') {
      setChatMessages((prevMessages) => [
        ...prevMessages, {
          id: Date.now() + 1,
          sender: "ai",
          text: apiResponse.data.result,
        ...
        >
          <div className="p-4 sm:p-6 border-b border-slate-200 dark:border-slate-700/60">
            ...
            <p className="text-sm text-center sm:text-left text-slate-500 dark:text-slate-400 mt-2 ml-0 sm:ml-[calc(2.5rem+1rem)] md:ml-[calc(3rem+1rem)]">
              Jelajahi dunia AI dan berbincang dengan karakter Alice!
            </p>
          </div>
            ...
            <div className="p-4 sm:p-6 space-y-6">
              <div className="bg-slate-100/70 dark:bg-slate-800/40 p-4 sm:p-5 rounded-lg border border-slate-200 dark:border-slate-700/60">
                <h3 className="text-lg font-semibold text-teal-700 dark:text-teal-300 mb-3 flex items-center">
                  ...
                  Cari Karakter Alice AI
                </h3>
                <form onSubmit={handleSearchCharacter} className="space-y-4">
                  <InputGroup
                    id="searchCharacter"
                    type="text"
                    placeholder="Ketik nama karakter... (Contoh: Miku)"
                    value={searchTerm}
                          ...
                          >
                            ...
                            <div className="flex-1 min-w-0">
                              ...
                              <span className="text-xs text-slate-400 dark:text-slate-500 block truncate">
                                Pesan: {character.chatSessionCount} | Disukai: {character.createdBy}
                              </span>
                            </div>
                          </li>
                        ))}
...
export default AliceAiChatPage;