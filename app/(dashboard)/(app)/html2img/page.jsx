...
const HtmlToImagePage = () => {
  ...
  // --- Styling Constants (aligned with ChatGroupPage & ApiSpecExplorerPage) ---
  ...
  const sectionCardClass = "bg-slate-100/70 dark:bg-slate-800/40 p-4 sm:p-5 rounded-lg border border-slate-200 dark:border-slate-700/60 shadow";
  ...
  useEffect(() => {
    // Open modal only if image is generated and no error/loading
    if (imageUrl && !error && !loading) {
      setShowImageModal(true);
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [imageUrl, error, loading]); // Added loading to deps
  ...
  const handleSubmit = async (e) => {
    ...
    setError(null);
    setImageUrl(null);
    setShowImageModal(false); // Close modal on new submission
  ...
  const handleDownloadImage = () => {
    ...
    link.href = imageUrl;
    // Attempt to get filename from URL or default
    const filename = imageUrl.substring(imageUrl.lastIndexOf('/') + 1).split('?')[0];
    link.download = filename || `html-image-${Date.now()}.png`; // More descriptive default name
        ...
        <ToastContainer
          ...
          toastClassName={(options) =>
            ...
            ${options?.type === 'success' ? 'bg-emerald-500 text-white' :
              ...
              options?.type === 'warn' ? 'bg-yellow-500 text-white' : // Changed to white text for warn
              'bg-sky-500 text-white'} dark:text-slate-100 text-sm p-3 m-2 rounded-lg shadow-md`
          }
        />
        <Card
          bodyClass="relative p-0 h-full overflow-hidden flex flex-col" // Added flex-col for consistent structure
        ...
        >
          ...
          <SimpleBar className="flex-grow max-h-[calc(100vh-220px)] md:max-h-[calc(100vh-200px)]">
            <div className="p-4 sm:p-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div className={sectionCardClass}>
                        ...
                        <Textarea
                            ...
                            required
                            className={`${inputBaseClass} font-mono`} // Combined with base class
                        />
                    ...
                    <div className="space-y-4">
                        <div className={sectionCardClass}>
                            ...
                            <Textinput
                                id="imageWidth"
                                type="number"
                                placeholder="Auto (lebar konten HTML)" // More informative placeholder
                                value={imageWidth}
                                min={1}
                                step={10}
                                onChange={(e) => setImageWidth(e.target.value)}
                                className={inputBaseClass} // Combined with base class
                            />
                            <p className="text-xs text-slate-500 dark:text-slate-400 mt-2">
                        ...
                        <div className={sectionCardClass}>
                            ...
                            <Textinput
                                id="imageHeight"
                                type="number"
                                placeholder="Auto (tinggi konten HTML)" // More informative placeholder
                                value={imageHeight}
                                min={1}
                                step={10}
                  ...
                  text={
                    loading ? (
                      <span className="flex items-center justify-center">
                        <Icon icon="svg-spinners:ring-resize" className="text-xl mr-2" /> {/* Consistent loading icon */}
                        Menghasilkan Gambar...
                      </span>
                    ) : (
                      <span className="flex items-center justify-center">
                        <Icon icon="ph:file-image-duotone" className="text-xl mr-2" /> {/* Consistent action icon */}
                        Hasilkan Gambar
                      </span>
                    )
                  }
              ...
              {imageUrl && !loading && (
                <div className={`mt-6 ${sectionCardClass}`}> {/* Re-used sectionCardClass */}
                  <h3 className={sectionTitleClass}> {/* Re-used sectionTitleClass */}
