...
// import useWidth from "@/hooks/useWidth"; // Tidak digunakan secara eksplisit, bisa dihapus jika tidak ada dependensi lain
import Card from "@/components/ui/Card";
import Button from "@/components/ui/Button";
import Textinput from "@/components/ui/Textinput"; // Asumsi komponen ini bisa menerima className untuk styling
...
const PasteManager = () => {
  ...
  const apiUrl = "/api/tools/paste/v1"; // Sesuaikan jika API URL berbeda
  ...
  const fetchPasteList = async () => {
    ...
    try {
      ...
      const errorMessage = err.response?.data?.error || "Gagal mengambil daftar paste";
  ...
  const handleCreatePaste = async () => {
    ...
    try {
      ...
      setSyntax("text");
      setExpireIn("");
      fetchPasteList(); // Muat ulang daftar setelah membuat
    } catch (err) {
  ...
  const handleGetPaste = async () => {
    ...
    try {
      ...
      const errorMessage = err.response?.data?.error || "Gagal mengambil paste atau paste tidak ditemukan/kadaluarsa.";
      dispatch(setError(errorMessage));
      setViewedPaste(null); // Pastikan viewedPaste bersih jika error
      toast.error(errorMessage);
    } finally {
  ...
  const handleDeletePaste = async (keyToDelete) => {
    ...
    try {
      const response = await axios.delete(`${apiUrl}?action=delete&key=${actualKey}`);
      ...
      fetchPasteList(); // Muat ulang daftar setelah menghapus
    } catch (err) {
  ...
  const handleClearAllPastes = async () => {
    if (window.confirm("Apakah Anda yakin ingin menghapus semua paste? Tindakan ini tidak dapat diurungkan.")) {
      ...
      try {
        ...
        const errorMessage = err.response?.data?.error || "Gagal menghapus semua paste";
        dispatch(setError(errorMessage));
        toast.error(errorMessage);
      } finally {
  ...
  const confirmDelete = (key, pasteTitle) => {
    ...
    if (window.confirm(`Apakah Anda yakin ingin menghapus paste "${titleDisplay}" (Key: ${key})?`)) {
  ...
  const inputBaseClass = "w-full bg-white text-slate-900 border border-slate-300 rounded-md px-3 py-2 text-sm focus:outline-none focus:ring-1 focus:ring-emerald-500 dark:bg-slate-700 dark:text-slate-200 dark:border-slate-600 placeholder-slate-400 dark:placeholder-slate-500";
  const labelClass = "block text-xs font-medium text-slate-600 dark:text-slate-300 mb-1";
  const sectionTitleClass = "text-base sm:text-lg font-medium text-emerald-700 dark:text-emerald-400 mb-3 flex items-center";
  const sectionCardClass = "bg-white dark:bg-slate-700/30 p-4 sm:p-5 rounded-lg shadow border border-slate-200 dark:border-slate-700/60";
  ...
  return (
    <>
        ...
        toastClassName={(o) =>
          `relative flex p-1 min-h-10 rounded-md justify-between overflow-hidden cursor-pointer ${
            o?.type === "success"
              ...
              : o?.type === "warning"
              ? "bg-yellow-500 text-white" // Menggunakan yellow untuk warning
      ...
      <div className="w-full px-2 sm:px-4 py-6">
        <Card
          bodyClass="relative p-0 flex flex-col h-full max-h-[calc(100vh-4rem)] overflow-hidden" // Adjusted for page padding
        ...
        >
          ...
          <div className="p-4 sm:p-6 border-b border-slate-200 dark:border-slate-700/60 flex-shrink-0">
            <div className="flex flex-col sm:flex-row items-center">
              <div className="w-10 h-10 flex items-center justify-center rounded-full bg-gradient-to-br from-teal-500 to-emerald-600 text-white shadow-md mb-2 sm:mb-0 sm:mr-4">
            ...
            <div className="p-4 sm:p-6 space-y-6">
              ...
              <div className={sectionCardClass}>
                ...
                <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                  <div>
                    <label htmlFor="pasteTitle" className={labelClass}>Judul (Opsional)</label>
                    <Textinput
                      id="pasteTitle"
                      type="text"
                      value={title}
                  ...
                  <div>
                    <label htmlFor="pasteSyntax" className={labelClass}>Syntax Highlighting</label>
                    <select
                      id="pasteSyntax"
                      className={inputBaseClass}
                      value={syntax}
                  ...
                  <div className="col-span-full">
                    <label htmlFor="pasteContent" className={labelClass}>Konten*</label>
                    <textarea
                      id="pasteContent"
                  ...
                  <div>
                    <label htmlFor="pasteExpireIn" className={labelClass}>Kadaluarsa (detik, opsional)</label>
                    <Textinput
                      id="pasteExpireIn"
                      type="number"
                      value={expireIn}
                  ...
                  <div className="sm:self-end">
                    <Button
                      ...
                      icon={loadingStates.create ? "svg-spinners:ring-resize" : "ph:floppy-disk-back-duotone"}
              ...
              <div className={sectionCardClass}>
                ...
                <div className="grid grid-cols-1 gap-4 sm:grid-cols-3 items-end">
                  <div className="sm:col-span-2">
                    <label htmlFor="viewKeyInput" className={labelClass}>Key Paste</label>
                    <Textinput
                      id="viewKeyInput"
                      type="text"
                      value={viewKey}
                  ...
                  <div>
                    <Button
                      ...
                      icon={loadingStates.getView ? "svg-spinners:ring-resize" : "ph:eye-duotone"}
                ...
                {loadingStates.getView && (
                    <div className="mt-4 p-4 bg-slate-100 dark:bg-slate-700/50 rounded-md text-center">
                        <Icon icon="svg-spinners:blocks-shuffle-3" className="text-3xl text-emerald-500 mx-auto mb-2" />
                        <p className="text-sm text-slate-600 dark:text-slate-300">Memuat detail paste...</p>
                    </div>
                )}
                {!loadingStates.getView && viewedPaste && (
                  <div className="mt-4 space-y-2 p-3 sm:p-4 bg-slate-100 dark:bg-slate-900/50 rounded-md border border-slate-200 dark:border-slate-700">
                    <h5 className="font-semibold text-emerald-600 dark:text-emerald-300 mb-2 text-sm">Detail Paste: "{viewedPaste.title || '(Tanpa Judul)'}"</h5>
                    <div className="text-xs text-slate-600 dark:text-slate-300 space-y-1">
                        <p><strong>Key:</strong> <code className="bg-slate-200 dark:bg-slate-600 px-1 rounded text-sky-700 dark:text-sky-300">{viewedPaste.key}</code></p>
                        <p><strong>Syntax:</strong> {viewedPaste.syntax}</p>
                        {viewedPaste.expiresAt && (
                            <p><strong>Kadaluarsa:</strong> {new Date(viewedPaste.expiresAt).toLocaleString()}</p>
                        )}
                        <p>
                    ...
                    <SimpleBar className="max-h-60">
                        <pre className="whitespace-pre-wrap text-sm bg-slate-200 dark:bg-slate-800 p-2 sm:p-3 rounded-md border border-slate-300 dark:border-slate-600 text-slate-800 dark:text-slate-100">{viewedPaste.content}</pre>
                    </SimpleBar>
                  </div>
                )}
                 {!loadingStates.getView && !viewedPaste && viewKey && ( // Added viewKey condition here to only show if search was attempted
                    <div className="mt-4 p-3 bg-yellow-50 dark:bg-yellow-900/30 border border-yellow-300 dark:border-yellow-700/50 rounded-md text-center">
                        <Icon icon="ph:warning-circle-duotone" className="text-2xl text-yellow-500 mx-auto mb-1" />
                        <p className="text-sm text-yellow-700 dark:text-yellow-300">Paste tidak ditemukan atau sudah kadaluarsa.</p>
                    </div>
                )}
              </div>
              
              {/* Section: Delete Paste */}
              <div className={sectionCardClass}>
                <label className={sectionTitleClass}>
                ...
                <div className="grid grid-cols-1 gap-4 sm:grid-cols-3 items-end">
                  <div className="sm:col-span-2">
                    <label htmlFor="deleteKeyInput" className={labelClass}>Key Paste</label>
                    <Textinput
                      id="deleteKeyInput"
                      type="text"
                      value={deleteKey}
                  ...
                  <div>
                    <Button
                      ...
                      icon={loadingStates.deleteByKey ? "svg-spinners:ring-resize" : "ph:trash-simple-duotone"}
              ...
              <div className={sectionCardClass}>
                <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-3 gap-2">
                  <label className={sectionTitleClass}>
                    <Icon icon="ph:list-bullets-duotone" className="mr-2 text-xl" />
                    Daftar Semua Paste
                  </label>
                  <Button
                    text={loadingStates.clearAll ? "Membersihkan..." : "Hapus Semua Paste"}
                    icon={loadingStates.clearAll ? "svg-spinners:ring-resize" : "ph:toilet-paper-duotone"}
                    ...
                    disabled={loadingStates.clearAll || loadingStates.list || !pastes || pastes.length === 0}
                    onClick={handleClearAllPastes}
                  />
                </div>
                <SimpleBar style={{ maxHeight: "400px" }} className="-mx-1"> {/* Negative margin to compensate item padding */}
                  {loadingStates.list ? (
                    <div className="text-center py-10">
                      <Icon icon="svg-spinners:blocks-shuffle-3" className="text-4xl text-emerald-500 mb-2 mx-auto" />
                      <p className="text-slate-500 dark:text-slate-400">Memuat daftar paste...</p>
                    </div>
                  ) : pastes && pastes.length > 0 ? (
                    <div className="space-y-3 px-1">
                      {pastes.map((paste) => (
                        <div
                          key={paste._id || paste.key} // Use key as fallback
                        ...
                        >
                          <div className="flex flex-col sm:flex-row justify-between items-start mb-1.5">
                            <div className="flex-1 mb-2 sm:mb-0">
                              <h6 className="text-emerald-700 dark:text-emerald-300 font-semibold text-sm truncate" title={paste.title || "(Tanpa Judul)"}>
                              ...
                              <div className="flex items-center gap-2 text-xs text-slate-500 dark:text-slate-400 mt-0.5">
                                ...
                                {paste.expiresAt && (
                                  <span className="text-amber-600 dark:text-amber-400 flex items-center">
                                    <Icon icon="ph:clock-countdown-duotone" className="inline mr-1 text-xs" />
                            ...
                            <div className="flex gap-1.5 self-start sm:self-center">
                              <Button icon="ph:copy-duotone" onClick={() => copyToClipboard(paste.key, "Key")} className="p-1.5 text-xs bg-sky-500 hover:bg-sky-600 text-white rounded" title="Salin Key" disabled={loadingStates.deleteListItem === paste.key}/>
                              <Button icon="ph:link-duotone" onClick={() => copyToClipboard(`${window.location.origin}/api/tools/paste/v1?action=get&key=${paste.key}&raw=true`, "Link")} className="p-1.5 text-xs bg-teal-500 hover:bg-teal-600 text-white rounded" title="Salin Link Raw" disabled={loadingStates.deleteListItem === paste.key}/>
                              <Button icon="ph:eye-duotone" onClick={() => { setViewKey(paste.key); handleGetPaste(); document.getElementById('viewKeyInput')?.focus();}} className="p-1.5 text-xs bg-indigo-500 hover:bg-indigo-600 text-white rounded" title="Lihat Paste Ini" />
                              <Button 
                                icon={loadingStates.deleteListItem === paste.key ? "svg-spinners:ring-resize" : "ph:trash-duotone"} 
                                onClick={() => confirmDelete(paste.key, paste.title)} 
                                className={`p-1.5 text-xs bg-red-500 hover:bg-red-600 text-white rounded ${loadingStates.deleteListItem === paste.key ? 'opacity-70 cursor-wait' : ''}`} 
                                title="Hapus Paste" 
                          ...
                          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-1 text-xs">
                            <code className="text-sky-600 dark:text-sky-400 bg-slate-100 dark:bg-slate-600/70 px-1.5 py-0.5 rounded text-xs truncate max-w-full sm:max-w-[calc(100%-100px)]" title={paste.key}>
                              Key: {paste.key}
                            </code>
                            <a
                    ...
                    <div className="text-center py-10">
                      <Icon icon="ph:files-duotone" className="text-4xl text-slate-400 dark:text-slate-500 mb-2 mx-auto" />
                      <p className="text-slate-500 dark:text-slate-400">Belum ada paste yang tersimpan.</p>
                    </div>
                  )}
                </SimpleBar>
              </div>
