...
const ApiSpecExplorerPage = () => {
  ...
  const fetchApiEndpoints = async (browsePath) => {
    ...
    try {
      ...
      if (browsePath === INITIAL_PATH) {
        ...
        setFileList(endpoints.sort((a,b) => a.name.localeCompare(b.name)));
      }
  ...
  const methodDisplayInfo = {
    GET:     { colorClass: "text-green-500 dark:text-green-400", icon: "ph:arrow-right-duotone" },
    POST:    { colorClass: "text-sky-500 dark:text-sky-400", icon: "ph:arrow-down-duotone" },
    PUT:     { colorClass: "text-yellow-500 dark:text-yellow-400", icon: "ph:pencil-duotone" },
  ...
  const inputBaseClass = "w-full bg-white dark:bg-slate-700/80 border border-slate-300 dark:border-slate-600 text-slate-900 dark:text-slate-100 rounded-md shadow-sm text-sm px-3 py-2 focus:ring-1 focus:ring-sky-500 focus:border-sky-500 outline-none placeholder:text-slate-400 dark:placeholder:text-slate-500";
            ...
            {bodyFields.map((f, i) => (
              <div key={f.id} className="grid grid-cols-12 gap-2 items-start">
                <div className="col-span-12 sm:col-span-4">
                  <input type="text" {...register(`requestBodyParams.${i}.key`)} placeholder="Key (opsional)" className={tryItInputBaseClass} title="Biarkan kosong jika mengirim JSON mentah di kolom Value" />
                </div>
  ...
  return (
    <div className="w-full px-2 sm:px-4 md:px-6 py-6">
      ...
      <Card bodyClass="relative p-0 h-full overflow-hidden" className="w-full border border-sky-500/50 dark:border-sky-600/70 rounded-xl shadow-lg bg-white text-slate-800 dark:bg-slate-800/50 dark:text-slate-100 backdrop-blur-sm bg-opacity-80 dark:bg-opacity-80">
        <div className="p-4 sm:p-6 border-b border-slate-200 dark:border-slate-700/60">
          <div className="flex flex-col sm:flex-row items-center justify-between">
            <div className="flex items-center mb-2 sm:mb-0">
              <div className="w-10 h-10 sm:w-11 sm:h-11 flex items-center justify-center rounded-full bg-gradient-to-br from-sky-500 to-blue-600 text-white shadow-md mr-3 shrink-0">
                <Icon icon="ph:plugs-connected-duotone" className="text-2xl sm:text-3xl" />
              </div>
              <h1 className="text-xl md:text-2xl font-bold bg-clip-text text-transparent bg-gradient-to-r from-sky-500 to-blue-500 text-center sm:text-left">
                API Explorer
              </h1>
            </div>
          ...
          <div className="mt-3 text-xs sm:text-sm text-slate-600 dark:text-slate-400">
            Path: <code className="bg-slate-100 dark:bg-slate-700 p-1 rounded text-sky-600 dark:text-sky-300 break-all">{currentBrowsePath === INITIAL_PATH ? "Daftar Tag API" : `Tag: ${currentBrowsePath}`}</code>
          </div>
        </div>

        {loadingList && !error && ( <div className="flex flex-col items-center justify-center p-10 min-h-[300px]"><Icon icon="svg-spinners:blocks-shuffle-3" className="text-5xl text-sky-500 mb-4" /><p className="text-lg font-medium text-slate-600 dark:text-slate-300">Memuat Daftar...</p></div>)}
          ...
          <div className="md:flex md:min-h-[calc(100vh-300px)] md:max-h-[calc(100vh-230px)]">
            <div className="w-full md:w-2/5 lg:w-1/3 border-r border-slate-200 dark:border-slate-700/60 bg-white dark:bg-slate-800/40 flex flex-col">
              <div className="p-3 sm:p-4 border-b border-slate-200 dark:border-slate-700/60">
                <input
                  type="text"
                  placeholder={currentBrowsePath === INITIAL_PATH ? "Cari tag..." : "Cari endpoint..."}
                  value={searchTerm}
                ...
                <div className="p-3 sm:p-2 space-y-0.5">
                  {paginatedItems.length > 0 ? paginatedItems.map((item) => (
                    <button
                      ...
                      title={`Nama: ${item.name}\nTipe: ${item.type === 'dir' ? 'Tag API' : `Endpoint API (${item.method})`}${item.type === 'dir' && item.endpointCount ? `\nJumlah Endpoint: ${item.endpointCount}` : ''}${item.type === 'endpoint' ? `\nPath: ${item.path}` : ''}`}
                      className={`w-full text-left flex items-center px-2.5 py-2 my-0.5 rounded-md hover:bg-sky-50 dark:hover:bg-sky-700/30 transition-colors duration-150 group ${showTryItModal && currentApiEndpoint?.id === item.id && item.type === 'endpoint' ? "bg-sky-100 dark:bg-sky-600/40 ring-1 ring-sky-400 dark:ring-sky-500" : ""}`}
                    >
                      <Icon
                      ...
                      />
                      <span className={`truncate text-sm ${showTryItModal && currentApiEndpoint?.id === item.id && item.type === 'endpoint' ? "text-sky-700 dark:text-sky-200 font-medium" : "text-slate-700 dark:text-slate-300 group-hover:text-slate-900 dark:group-hover:text-slate-100"}`}>
                        {item.name}
                      </span>
            ...
            <div className="hidden md:flex md:w-3/5 lg:w-2/3 bg-slate-50 dark:bg-slate-900/30 items-center justify-center">
                <div className="text-center text-slate-500 dark:text-slate-400 p-10">
                  <Icon icon="ph:cursor-click-duotone" className="text-7xl mb-4 opacity-60" />
                  <p className="text-lg">Pilih endpoint API dari panel kiri untuk mencoba.</p>
                  <p className="text-sm mt-1">Detail dan form interaktif akan muncul di modal.</p>
                </div>
            </div>
          </div>
        )}
          ...
          footerContent={
            <div className="flex flex-col sm:flex-row justify-end w-full gap-3">
              ...
              <Button
                ...
                className="bg-gradient-to-r from-sky-500 to-blue-500 hover:from-sky-600 hover:to-blue-600 text-white w-full sm:w-auto px-4 py-2 text-sm shadow-sm hover:shadow-md flex items-center justify-center"
                  ...
                  <div className="border border-slate-200 dark:border-slate-700/80 rounded-lg overflow-hidden">
                    ...
                    {(tryItResponseType === "document" || tryItResponseType === "blob") && tryItResponse?.url && (
                      <div className="p-3 bg-slate-100 dark:bg-slate-700/50">
                        ...
                        <a href={tryItResponse.url} download={tryItResponse.filename || "download"} className="inline-flex items-center px-3 py-1.5 bg-sky-500 hover:bg-sky-600 text-white text-xs font-medium rounded-md transition-colors"> <Icon icon="ph:download-simple-duotone" className="mr-1.5 text-base" /> Unduh Berkas </a>
