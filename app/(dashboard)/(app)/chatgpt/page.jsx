...
import InputGroup from "@/components/ui/InputGroup"; // Menggunakan InputGroup untuk textarea
...
const AiChatgptPage = () => {
  ...
  const defaultAiCharacter = {
    id: "chatgpt-default",
    name: "ChatGPT",
    // Menggunakan ikon placeholder yang lebih netral, atau sesuaikan path jika ada ikon yang lebih cocok
    characterImageUrl: "/assets/images/users/user-2.jpg", // Ganti jika perlu
  };

  const userAvatarUrl = "/assets/images/users/user-1.jpg"; // Pastikan path ini ada
  ...
  const callChatgptApi = async (url, method = "GET", body = null) => {
    ...
    try {
      ...
      if (data.result || response.ok) { // Check response.ok as fallback
    ...
    if (data.success && data.data && data.data.result) {
      ...
      setChatMessages((prevMessages) => [
        ...prevMessages,
        {
          ...
          avatar: defaultAiCharacter.characterImageUrl, // Bisa diganti dengan ikon sistem error
        },
      ]);
    }
  };
  ...
  return (
    <>
      ...
      <div className="w-full px-2 sm:px-4 py-6">
        <Card
          bodyClass="relative p-0 h-full overflow-hidden" // Mengikuti OpenApiSpecManagerPage
          className="w-full border border-emerald-500/50 dark:border-emerald-600/70 rounded-xl shadow-lg bg-white text-slate-800 dark:bg-slate-800/50 dark:text-slate-100 backdrop-blur-sm bg-opacity-80 dark:bg-opacity-80" // Mengikuti OpenApiSpecManagerPage
        >
          {/* Header */}
          <div className="p-4 sm:p-6 border-b border-slate-200 dark:border-slate-700/60">
            <div className="flex flex-col sm:flex-row items-center">
              ...
              <div>
                <h4 className="text-xl font-bold text-center sm:text-left bg-clip-text text-transparent bg-gradient-to-r from-teal-500 to-emerald-500">
          ...
          {/* Chat Area & Input */}
          {/* Penyesuaian tinggi mungkin diperlukan berdasarkan layout halaman keseluruhan */}
          <div className="p-4 sm:p-6 bg-slate-50 dark:bg-slate-800/30 flex flex-col h-[calc(100vh-220px)] md:h-[calc(100vh-190px)]">
                      ...
                      {(msg.sender === "ai" || msg.sender === "system") && (
                        <img
                          ...
                          className={`w-8 h-8 rounded-full mr-2 sm:mr-3 border-2 shadow-sm ${
                            msg.sender === "ai" ? "border-emerald-400" : "border-red-400" // Warna border disesuaikan
                          }`}
                        />
                      )}
                      <div
                        className={`p-2.5 sm:p-3 rounded-lg shadow-md max-w-[75%] sm:max-w-[70%] md:max-w-[65%] ${
                          msg.sender === "user"
                            ? "bg-gradient-to-br from-emerald-500 to-teal-600 text-white rounded-br-none" // User message style
                            : msg.sender === "ai"
                            ? "bg-white text-slate-800 rounded-bl-none dark:bg-slate-700 dark:text-slate-100" // AI message style
                            : "bg-red-100 text-red-700 rounded-bl-none dark:bg-red-900/50 dark:text-red-200" // System message style
                        }`}
                      >
            ...
            <form onSubmit={handleSendMessage} className="mt-auto pt-3 border-t border-slate-200 dark:border-slate-700/60">
              <InputGroup
                ...
                rows="2" // Bisa disesuaikan, misal 1 atau 2
                className="bg-white border-slate-300 text-slate-900 rounded-lg focus:ring-1 focus:ring-emerald-500 focus:border-emerald-500 dark:bg-slate-700 dark:border-slate-600 dark:text-slate-200 placeholder-slate-400 dark:placeholder-slate-500 text-sm p-2.5" // Style input
                disabled={loading}
                onKeyDown={(e) => {
              ...
              <Button
                ...
                className="w-full mt-2 sm:mt-3 text-white bg-gradient-to-r from-emerald-500 to-sky-500 hover:from-emerald-600 hover:to-sky-600 rounded-lg py-2.5 sm:py-2.5 font-medium transition-all duration-300 transform hover:scale-[1.01] shadow-md hover:shadow-lg" // Style tombol kirim
