...
const INITIAL_PATH = "__TAGS_ROOT__"; // Represents the root "directory" of tags

const ApiSpecExplorerPage = () => {
  // State for API specification and navigation
  const [apiSpecProcessedData, setApiSpecProcessedData] = useState(null); // Stores the processed spec (tags, endpointsByTag)
  const [fileList, setFileList] = useState([]); // Current list of items (tags or endpoints) to display
  const [currentBrowsePath, setCurrentBrowsePath] = useState(INITIAL_PATH); // The current "directory" being viewed
  const [pathHistory, setPathHistory] = useState([INITIAL_PATH]); // Stack for navigation history

  // Loading and error states for fetching the spec
  ...
  const [tryItResponseUrl, setTryItResponseUrl] = useState(null); // For blob/file URLs
  const [tryItLoading, setTryItLoading] = useState(false);
  const [tryItError, setTryItError] = useState(null);
  const [responseCopied, setResponseCopied] = useState(false);

  // React Hook Form for Try-It modal
  const { register, handleSubmit, reset, setValue, control, watch } = useForm();
  const watchedMethod = watch("method");

  // Field Arrays for dynamic parameters in Try-It modal
  ...
  // --- Helper to process OpenAPI spec into a navigatable structure ---
  const processOpenApiSpec = useCallback((data) => {
    ...
    Object.entries(paths).forEach(([path, methods]) => {
      Object.entries(methods).forEach(([method, details]) => {
        if (method.toLowerCase() === 'parameters') return; // Skip global parameters object

        const tagName = details.tags?.[0] || "Miscellaneous"; // Default tag if none specified
        if (!groupedByTag[tagName]) {
          groupedByTag[tagName] = {
            description: specTags.find(t => t.name === tagName)?.description || "Endpoints in this group.",
            endpoints: []
          };
        }
        groupedByTag[tagName].endpoints.push({
          ...
          type: 'endpoint', // Mark as an endpoint "file"
          details: details,
          parentTag: tagName,
          id: `${tagName}-${path}-${method.toUpperCase()}` // Unique ID for each endpoint
        });
      });
    });
    ...
    for (const tagName in groupedByTag) {
        groupedByTag[tagName].endpoints.sort((a, b) => {
          ...
          if (a.path > b.path) return 1;
          // Then by method for paths with same name
    ...
    // Sort tags (directories)
    const sortedTagNames = Object.keys(groupedByTag).sort((a, b) => {
        const indexA = tagOrder.indexOf(a);
        const indexB = tagOrder.indexOf(b);
        if (indexA !== -1 && indexB !== -1) return indexA - indexB; // Prioritize defined order
        if (indexA !== -1) return -1; // Keep defined tags at top
        if (indexB !== -1) return 1;
        if (a === "Miscellaneous") return 1; // "Miscellaneous" last
    ...
    const tags = sortedTagNames.map(tagName => ({
      name: tagName,
      type: 'dir', // Mark as a directory
      path: tagName, // The "path" is its name for navigation
      id: tagName, // Unique ID for each tag
  ...
  // --- Fetches and processes OpenAPI spec once, then navigates ---
  const loadAndNavigateApiSpec = useCallback(async (path) => {
    setLoadingList(true);
    setError(null);
    setFileList([]);
    setCurrentPage(1);
    ...
    try {
      let processedData = apiSpecProcessedData;
      if (!processedData) {
        // Only fetch if data is not already processed
        ...
        setApiSpecProcessedData(processedData); // Store processed data
      }

      // Determine what to display based on currentBrowsePath
      if (path === INITIAL_PATH) {
        setFileList(processedData.tags); // Display all tags as directories
      } else {
        const endpoints = processedData.endpointsByTag[path]?.endpoints || [];
        setFileList(endpoints); // Display endpoints for the selected tag
      }
  ...
  }, [apiSpecProcessedData, processOpenApiSpec]);

  // Initial load of API spec and navigation
  useEffect(() => {
    loadAndNavigateApiSpec(INITIAL_PATH);
  }, [loadAndNavigateApiSpec]);
  ...
  const handleItemClick = async (item) => {
    if (item.type === "dir") {
      // Navigate into a "directory" (tag)
      setPathHistory(prev => {
        // Prevent duplicate path if already at that path (e.g., direct URL access)
        if (prev[prev.length - 1] === item.path) return prev;
        return [...prev, item.path];
      });
      setCurrentBrowsePath(item.path); // Triggers re-render via useEffect
      setSearchTerm(""); // Clear search when navigating
      setCurrentPage(1); // Reset pagination
    } else if (item.type === "endpoint") {
      // Open the Try-It modal for an "endpoint file"
      openTryItModal(item);
    }
  };
  ...
  const handleGoBack = () => {
    if (pathHistory.length > 1) {
      ...
      newPathHistory.pop(); // Remove current path
      const previousPath = newPathHistory[newPathHistory.length - 1]; // Get the parent path
      setPathHistory(newPathHistory);
      setCurrentBrowsePath(previousPath); // Triggers re-render via useEffect
      setSearchTerm(""); // Clear search when navigating back
      setCurrentPage(1); // Reset pagination
    }
  };

  // --- Display Info for HTTP Methods ---
  ...
  // --- Styling Constants (aligned with previous theme) ---
  ...
  const openTryItModal = (apiEndpoint) => {
    ...
    removePath(); removeQuery(); removeHeader(); removeBody(); // Clear existing fields
    ...
    // Populate Request Body (more robust parsing for examples/schemas)
      ...
      if (requestBodyContent.example) {
        try {
          ...
          if (typeof exampleData === 'object' && exampleData !== null && !Array.isArray(exampleData)) {
            // If example is an object, populate as key-value pairs
            ...
            // If example is raw JSON string or non-object, treat as single raw body field
          ...
          // Fallback if example parsing fails
        ...
        // Populate from schema properties
        ...
        // Default empty JSON body if no example or schema properties
        ...
        // For other content types, add a single raw body field
  ...
  const executeTryIt = async (formData) => {
    ...
    if (["POST", "PUT", "PATCH"].includes(methodToExecute) && formData.requestBodyParams?.length > 0) {
        ...
        // Set Content-Type header if not explicitly set by user and it's JSON
        if (!headers["Content-Type"] && !explicitContentType && effectiveContentType === "application/json") {
            headers["Content-Type"] = "application/json";
        }

        let bodyObject = {};
        // Check if it's a single raw JSON body input (empty key)
        ...
        if (effectiveContentType === "application/json") {
            if (isRawJsonBodyInput) {
                try { finalBody = formData.requestBodyParams[0].value || "";
                      if (finalBody.trim() !== "") { JSON.parse(finalBody); } // Validate JSON if not empty
                }
                ...
                formData.requestBodyParams.forEach(p => {
                    if (p.key) {
                        try {
                            ...
                            // Check if numeric, not starting with 0 (unless just "0"), and not too long (to prevent large number parsing issues)
                            else if (!isNaN(p.value) && p.value.trim() !== "" && !(p.value.startsWith("0") && p.value.length > 1) && p.value.length < 16 ) bodyObject[p.key] = Number(p.value);
                            else bodyObject[p.key] = p.value; // Treat as string by default
                        } catch (e) {
        ...
        } else { // For other content types (e.g., text/plain, application/xml)
  ...
  return (
    <div className="w-full px-2 sm:px-4 md:px-6 py-6">
      {/* ToastContainer with consistent theming */}
      ...
      <Card
        bodyClass="relative p-0 h-full overflow-hidden flex flex-col" // Changed to flex-col to better manage header and content area
      ...
      >
        ...
        <div className="p-4 sm:p-6 border-b border-slate-200 dark:border-slate-700/60 flex-shrink-0">
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between">
            {/* Icon, Title, and Subtitle Container */}
          ...
          <div className="mt-3 text-xs sm:text-sm text-slate-600 dark:text-slate-400">
            Path: <code className="bg-slate-100 dark:bg-slate-700 p-1 rounded text-teal-600 dark:text-teal-300 break-all">
              {pathHistory.map((pathSegment, index) => (
                <Fragment key={index}>
                  {index > 0 && " / "}
                  {pathSegment === INITIAL_PATH ? "Root" : pathSegment}
                </Fragment>
              ))}
            </code>
          </div>
        ...
        {loadingList && !error && (
          <div className="flex flex-col items-center justify-center p-10 min-h-[300px]">
            ...
            <p className="text-lg font-medium text-slate-600 dark:text-slate-300">Memuat Spesifikasi API...</p>
          </div>
        )}
        {error && !loadingList && (
          <div className="flex flex-col items-center justify-center p-10 min-h-[300px] bg-red-50 dark:bg-red-800/20 rounded-b-xl">
            ...
            <p className="text-lg font-semibold text-red-700 dark:text-red-300">Gagal Memuat</p>
            <p className="text-sm text-red-600 dark:text-red-400 mt-2 text-center max-w-xl">{error}</p>
          </div>
        )}

        {/* Main Content Area (List of Tags/Endpoints) */}
        {!loadingList && !error && (
          <div className="flex-grow flex flex-col"> {/* flex-grow to take remaining height */}
            ...
            <SimpleBar className="flex-grow min-h-0"> {/* flex-grow and min-h-0 make it scrollable */}
              <div className="p-3 sm:p-2 space-y-0.5">
                {paginatedItems.length > 0 ? (
                  ...
                  <div className="p-4 text-center text-slate-500 dark:text-slate-400">
                    <Icon icon={currentBrowsePath === INITIAL_PATH ? "ph:tags-duotone" : "ph:plugs-duotone"} className="mx-auto text-4xl opacity-70 mb-2"/>
                    <p className="text-sm">{searchTerm ? "Tidak ada yang cocok dengan pencarian Anda." : (currentBrowsePath === INITIAL_PATH ? "Tidak ada tag API yang ditemukan dalam spesifikasi." : "Tidak ada endpoint di tag ini.")}</p>
                    <p className="text-xs mt-1 text-slate-400 dark:text-slate-500">Pastikan spesifikasi OpenAPI Anda valid dan memiliki tags/paths.</p>
                  </div>
                )}
              </div>
            </SimpleBar>
            {totalPages > 1 && (
              <div className="p-3 border-t border-slate-200 dark:border-slate-700/60 flex justify-between items-center text-xs flex-shrink-0">
      ...
      {currentApiEndpoint && (
        <Modal
          ...
          className="max-w-2xl border border-teal-500/50 dark:border-teal-600/70 rounded-xl shadow-lg bg-white text-slate-800 dark:bg-slate-800/50 dark:text-slate-100 backdrop-blur-sm bg-opacity-80 dark:bg-opacity-80"
          footerContent={
