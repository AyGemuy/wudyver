...
const AkinatorPage = () => {
  ...
  const startSession = async () => {
    setLoading(true);
    const res = await fetch('/api/game/akinator?action=start');
  ...
  const answerQuestion = useCallback(
    async (answer) => {
      setLoading(true);
      const res = await fetch(`/api/game/akinator?action=answer&sessionId=${sessionId}&answer=${answer}`);
      const data = await res.json();
      setQuestion(data.question);
  ...
  const cancelAnswer = async () => {
    setLoading(true);
    const res = await fetch(`/api/game/akinator?action=cancel&sessionId=${sessionId}`);
    const data = await res.json();
    setQuestion(data.question);
    setProgress(data.progress);
    setLoading(false);
  ...
  useEffect(() => {
    if (sessionId && !isWin) {
      const interval = setInterval(async () => {
        await answerQuestion(Math.floor(Math.random() * 5));
      }, 5000);
      return () => clearInterval(interval);
    }
  }, [sessionId, isWin, answerQuestion]);
