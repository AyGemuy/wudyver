...
const SignUp = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await fetch(`/api/auth/signup?email=${encodeURIComponent(email)}&password=${encodeURIComponent(password)}`, {
        method: 'GET',
      });
      ...
      // Jika berhasil sign up, arahkan ke halaman login
      if (data.status === 201) {
        setTimeout(() => {
          window.location.href = '/authentication/sign-in'; // Redirect ke halaman sign-in
