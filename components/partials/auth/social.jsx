...
const Social = () => {
  ...
  const copyToClipboard = async (text, type) => {
    if (!text) {
      ...
      alert(`Gagal menyalin ${type}. Nilai tidak ditemukan.`);
      return;
    }
    try {
      ...
      alert('Gagal menyalin. Pastikan browser Anda mendukung fitur ini atau coba lagi.');
    }
  };
    ...
    <>
      <div className="w-full">
        <Button
          ...
          className="w-full text-white bg-gradient-to-r from-teal-500 to-cyan-600 hover:from-teal-600 hover:to-cyan-700 shadow-md hover:shadow-lg transition duration-300 py-2.5 rounded-xl text-sm font-semibold flex items-center justify-center space-x-2"
          type="button"
        >
          <Icon icon="solar:user-circle-plus-duotone" className="text-xl" />
        ...
        <div className="border-t border-slate-200 dark:border-slate-700/60 mt-4 pt-4 md:mt-5 md:pt-5">
          <div className="space-y-5 px-4 pb-4 md:px-6 md:pb-6 text-sm text-slate-700 dark:text-slate-300">
            <p className="text-center text-base text-slate-700 dark:text-slate-200">
              Silakan gunakan kredensial berikut untuk melanjutkan:
            </p>
