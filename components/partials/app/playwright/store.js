// redux/slices/playwrightSlice.js
import { createSlice, createAsyncThunk } from '@reduxjs/toolkit';
import { toast } from 'react-toastify';

export const runPlaywrightCode = createAsyncThunk(
  'playwright/run',
  async ({ sourceCode, timeout }, { rejectWithValue }) => {
    try {
      const res = await fetch('/api/tools/playwright', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ code: sourceCode, timeout }),
      });
...
const playwrightSlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder
      .addCase(runPlaywrightCode.pending, (state) => {
        state.loading = true;
        state.error = null;
        state.output = '';
      })
