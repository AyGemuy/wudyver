...
export async function middleware(req) {
  ...
  const isInternalRequest = referer.includes(process.env.DOMAIN_URL) || req.headers.get("x-requested-with") === "XMLHttpRequest";
  const dashboardRoutes = ["dashboard", "playground", "docs", "pages"];
  if (!dashboardRoutes.some(route => pathname.includes(route)) && !isInternalRequest) {
